#!/bin/sh
# Owner-authorized physical RUN choreography. Its Python capture entrypoint
# enforces 27.653 s before the first t1.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v16_corrected_view_contact.py
DEPLOY=build/c2.3/v1.6-defstruct-phase-c/deployment.json
OUT=${OUT:-build/c2.3/v1.6-defstruct-closing-session/d2-corrected-view-quiet-appointment}
PRODUCT=$OUT/diagnostic-link82-corrected-view-b5c3.prg
...
if [ "$ACTION" = dry-run ]; then
  python3 "$PY" check
  python3 "$PY" selftest
  echo "D2 CORRECTED VIEW DRY RUN PASS samples=3 first-t1-min=27.653s recontact=authorized"
  exit 0
fi
...
if [ "$ACTION" = capture ]; then
  [ -e "$OUT/stage.ready" ] || {
    echo "corrected-view appointment is not staged" >&2; exit 3;
  }
  [ ! -e "$OUT/capture.consumed" ] || {
    echo "corrected-view capture already consumed" >&2; exit 3;
  }
...
[ ! -e "$OUT/stage.consumed" ] || {
  echo "corrected-view stage already consumed" >&2; exit 3;
}
: > "$OUT/stage.consumed"
run_m65 -F
sleep 5
...
run_m65 -H -@ "$SENTINEL@0x0000b5c3"
readback 0x0000b5c3 1 "$OUT/witness-underlay-before-run.bin"
cmp "$SENTINEL" "$OUT/witness-underlay-before-run.bin"
run_m65 -r
sleep 3
screen launch-ready
...
PY
python3 "$PY" prelaunch --device "$DEVICE"
: > "$OUT/stage.ready"
echo "D2 CORRECTED VIEW STAGE READY: type RUN and press RETURN physically."
