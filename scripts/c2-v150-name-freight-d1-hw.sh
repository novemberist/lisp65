#!/bin/sh
# Fresh owner-guided D1 for the v1.5 name-freight successor media.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
CONFIG=config/c2-v150-name-freight-d1.json
PY=tools/host-lisp/c2_v150_name_freight_d1.py
OUT=${OUT:-build/c2.3/v1.5.0-name-freight-d1}
...
PRODUCT_REMOTE=V15NFR.D81
LIBRARY_REMOTE=V15NFLIB.D81
...
if [ "$ACTION" = stage ]; then
  [ ! -e "$OUT/contact.consumed" ] || {
    echo "fresh v1.5 name-freight D1 contact already consumed" >&2; exit 3;
  }
...
python3 "$PY" record
echo "D1 GREEN. D2-D5 preparation is now permitted; none has run."
