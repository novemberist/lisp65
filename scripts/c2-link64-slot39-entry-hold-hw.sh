#!/bin/sh
# One-shot nonpromotable first-entry hold for Link-64 Session slot 39.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-30}
WAIT=${WAIT:-28}
OUT=build/c2.2/hardware-link64-slot39-entry-hold-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_link64_slot39_entry_capture.py
M65=$TOOLS/m65
...
deploy-and-arm)
  [ ! -e "$OUT/hardware-run.started" ] || {
    echo "Link-64 entry-hold deployment is one-shot" >&2
    exit 3
  }
...
if "lisp65>" not in text or "*** vm:" in text:
    raise SystemExit("Link-64 entry hold did not reach a clean REPL")
PY
...
capture)
  [ -e "$OUT/hardware-run.started" ] || {
    echo "Link-64 entry-hold identity was not deployed" >&2
    exit 3
  }
  capture_set() {
    cs_index=$1
...
(out / "capture-times.json").write_text(json.dumps({
    "format": "lisp65-Link64-slot39-entry-hold-times-v1",
    "interval_seconds": [0, 1, 5],
    "captures": rows,
}, indent=2, sort_keys=True) + "\n")
PY

  run_m65 --screenshot="$OUT/entry-hold-screen.png" \
    > "$OUT/entry-hold-screen.ansi.txt"
  python3 - "$OUT/entry-hold-screen.ansi.txt" \
    "$OUT/entry-hold-screen.txt" <<'PY'
from pathlib import Path
import re
import sys
raw = Path(sys.argv[1]).read_text(errors="ignore")
...
PY
  python3 "$PY" evaluate
  echo "Link-64 first-entry witness captured; diagnostic identity retired."
  ;;
esac
