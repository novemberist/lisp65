#!/bin/sh
# One-contact non-promotable Link-90 VM fault/CPU VIC-geometry capture.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v14_link90_vm_debug_identity.py
BASE=build/post-promotion/v14/link90-vm-debug-identity
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "diagnostic hardware contact already consumed" >&2; exit 3;
}
mkdir -p "$OUT"
...
state_address=$(($(jq -r '.symbols.lisp65_runtime_state.address' "$DEPLOY")))
terminal=$(jq -r '.terminal_value' "$DEPLOY")
elapsed=0
state=0
while [ "$elapsed" -lt 120 ]; do
  ...
  case "$state" in
    3|225|226|228|229)
      echo "diagnostic identity reached wrong terminal state $state" >&2; exit 1 ;;
  esac
  sleep 2
  elapsed=$((elapsed + 2))
done
[ "$state" = "$terminal" ] || {
  echo "diagnostic identity did not reach VM error state" >&2; exit 1;
}

jq -r '.symbols | to_entries[] | [.key,.value.address,.value.bytes] | @tsv' \
  "$DEPLOY" | while IFS="$(printf '\t')" read -r name address bytes; do
    [ "$name" = lisp65_runtime_state ] && continue
    readback "$((address))" "$bytes" "$OUT/$name.bin"
  done
