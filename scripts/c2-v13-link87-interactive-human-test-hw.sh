#!/bin/sh
# Final physical-keyboard acceptance for the Link-87 interactive Ship sample.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
CONFIG=config/c2-ship-builder-v1-link87-interactive-human-test.json
OUT=build/ship-builder/v13/link87-interactive-human-test/run
...
[ "$image_sha" = "$expected_sha" ] || {
  echo "Link-87 interactive image SHA drift" >&2; exit 3;
}

if [ "$ACTION" = dry-run ]; then
  [ -f "$image" ]
  echo "DRY-RUN: cold reset -> fresh BASIC -> exact committed Link-87 D81"
  echo "ASSERT: Runtime state 2 follows three independently sampled clock deltas"
  echo "HANDS OFF: no monitor traffic after state 2"
  echo "HUMAN: type Ada and RETURN on the physical keyboard"
  echo "CAPTURE: require state 3, result 0x0028 and Hello, Ada!"
  exit 0
fi
