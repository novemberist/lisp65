#!/bin/sh
# One-contact non-promotable Link-90 post-refill opcode-view witness.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v14_link90_opcode_view_witness.py
BASE=build/post-promotion/v14/link90-opcode-view-witness
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "opcode-view hardware contact already consumed" >&2; exit 3;
}
mkdir -p "$OUT"
...
[ "$state" = 227 ] || {
  echo "opcode-view identity did not reach VM error state: $state" >&2; exit 1;
}
witness_address=$(($(jq -r '.witness.address' "$DEPLOY")))
witness_bytes=$(jq -r '.witness.bytes' "$DEPLOY")
readback "$witness_address" "$witness_bytes" "$OUT/opcode_view_witness.bin"
python3 "$PY" analyze
