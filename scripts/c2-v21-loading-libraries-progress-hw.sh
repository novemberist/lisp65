#!/bin/sh
# Autonomous Link-107 CPU-transport progress-ring contact.
set -eu

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
cd "$ROOT"
ACTION=${1:-dry-run}
SESSION=config/c2-v21-loading-libraries-progress-session.json
RING=tools/host-lisp/c2_v21_loading_libraries_progress_rebind.py
CONTACT=tools/host-lisp/c2_v21_loading_libraries_progress_contact.py
OUT=build/c2.3/v2.1-loading-libraries-progress/contact
...
if [ "$ACTION" = dry-run ]; then
  echo "LINK-107 RING CONTACT DRY-RUN PASS"
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "Link-107 autonomous ring contact already consumed" >&2; exit 3;
}
...
echo "LINK-107 RING CONTACT COMPLETE; CPU remains stopped; D1-D5 remain closed"
