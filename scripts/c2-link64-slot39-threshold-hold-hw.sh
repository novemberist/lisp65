#!/bin/sh
# One-shot, fully rebound, nonpromotable Link-64 Slot-39 threshold hold.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-30}
WAIT=${WAIT:-28}
OUT=build/c2.2/hardware-link64-slot39-threshold-hold-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_link64_slot39_threshold_hold.py
M65=$TOOLS/m65

[ "$#" -eq 1 ] || {
  echo "usage: $0 <deploy-and-arm|capture>" >&2
  exit 2
}
ACTION=$1
case "$ACTION" in
  deploy-and-arm|capture) ;;
  *) echo "usage: $0 <deploy-and-arm|capture>" >&2; exit 2 ;;
esac
...
deploy-and-arm)
  [ ! -e "$OUT/hardware-run.started" ] || {
    echo "Link-64 threshold-hold hardware deployment is one-shot" >&2
    exit 3
  }
  ...
  # The current m65 autorun path can leave its generated BASIC `run:` pending.
  # Submit only that exact state; never type into an already-running product.
  sleep 3
...
if "lisp65>" not in text or "*** vm:" in text:
    raise SystemExit("Link-64 threshold hold did not reach a clean REPL")
PY

  # A naked $C8C2 breakpoint is forbidden: overlay VMAs do not identify a
  # slice. The sealed operand patch itself is the Slot-39-qualified hold.
  # Type the form without RETURN; the operator submits it physically.
  run_m65 -t "$(jq -r '.test.form' "$DEPLOY")"
  echo "Form armed. Press physical RETURN once; run capture after it hangs."
  ;;

capture)
  [ -e "$OUT/hardware-run.started" ] || {
    echo "Link-64 threshold identity was not deployed" >&2
    exit 3
  }
  ...
  mkdir "$cs_dir"
  readback 0x00000017 16 "$cs_dir/poll-state.bin"
...
(out / "capture-times.json").write_text(json.dumps({
    "format": "lisp65-Link64-slot39-threshold-hold-times-v1",
...
PY
  ...
  python3 "$PY" evaluate
  echo "Link-64 threshold witness captured; diagnostic identity retired."
  ;;
esac
