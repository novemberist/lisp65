...
__attribute__((used)) volatile uint8_t hw_chain_write_pass;
__attribute__((used)) volatile uint8_t hw_chain_write_total;
__attribute__((used)) volatile uint8_t hw_chain_write_results[CASES];
__attribute__((used)) volatile uint16_t hw_chain_write_got[CASES];
__attribute__((used)) volatile uint16_t hw_chain_write_want[CASES];
...
static void record(uint8_t idx, uint8_t ok, uint16_t got, uint16_t want) {
    hw_chain_write_results[idx] = ok ? 1 : 0;
    hw_chain_write_got[idx] = got;
    hw_chain_write_want[idx] = want;
    hw_chain_write_total++;
    if (ok) hw_chain_write_pass++;
}
...
static void fill_first_sector(void) {
    ...
    for (i = 0; i < 254; i++)
        scratch[2 + i] = (uint8_t)m3_chain_src[i];
}
...
static void fill_second_sector(uint16_t len) {
    ...
    for (i = 0; i < tail; i++)
        scratch[2 + i] = (uint8_t)m3_chain_src[254u + i];
}
...
static void show_result(void) {
    uint8_t pass = (hw_chain_write_pass == hw_chain_write_total);
    hw_border(pass ? COLOR_GREEN : COLOR_RED);
    puts_scr("chain write ");
    puts_scr(pass ? "pass " : "fail ");
    put_u16(hw_chain_write_pass);
    scr_putc('/');
    ...
    put_u16(payload_len());
    scr_putc('\n');
    puts_scr("bam count follows mounted image\n");
}
...
int main(void) {
    ...
    scr_init();
    hw_chain_write_pass = 0;
    hw_chain_write_total = 0;
    run_m3();
    show_result();
    for (;;) { }
    return 0;
