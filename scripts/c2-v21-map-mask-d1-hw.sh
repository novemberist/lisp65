#!/bin/sh
# Crossing-free Link-109 D1. No automated access after product mount.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
CONFIG=config/c2-v150-v21-map-mask-far-device-session.json
PY=tools/host-lisp/c2_v21_map_mask_d1.py
OUT=${OUT:-build/c2.3/v2.1-map-mask-d1}
...
PRODUCT_REMOTE=V21L109.D81
LIBRARY_REMOTE=V21L109L.D81
...
if [ "$ACTION" = dry-run ]; then
  ...
  echo "EXPECT: STAGING / HEAP / LIBRARIES / WORKBENCH 1.5.0 / prompt"
  exit 0
fi
...
if [ "$ACTION" = stage ]; then
  [ ! -e "$OUT/contact.consumed" ] || {
    echo "Link-109 D1 contact already consumed" >&2; exit 3;
  }
...
[ -e "$OUT/owner-observation-awaiting" ] \
  && [ ! -e "$OUT/owner-terminal-confirmed" ] || {
    echo "Link-109 D1 owner-confirmation state invalid" >&2; exit 3;
  }
printf '%s\n' \
