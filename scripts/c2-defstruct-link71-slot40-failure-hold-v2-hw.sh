#!/bin/sh
# One-shot identity-preserving Slot-40 error-edge discriminator for Link 71.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-35}
WAIT=${WAIT:-32}
OUT=build/post-promotion/link71-defstruct-header-crc-domain/slot40-failure-hold-v2-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_defstruct_link71_slot40_failure_hold_v2.py
PC=tools/host-lisp/c2_defstruct_link71_slot40_pc_capture_v2.py
M65=$TOOLS/m65
...
deploy)
  [ ! -e "$OUT/hardware-run.started" ] || {
    echo "Link-71 Slot-40 v2 diagnostic deployment is one-shot" >&2
    exit 3
  }
  ...
  grep -q 'lisp65>' "$OUT/boot.txt" &&
    ! grep -q '\*\*\* vm:' "$OUT/boot.txt" || {
      echo "Link-71 Slot-40 v2 diagnostic did not reach a clean REPL" >&2
      exit 3
    }
  echo "Slot-40 v2 diagnostic ready."
  ;;
arm)
  [ -e "$OUT/hardware-run.started" ] || {
    echo "Link-71 Slot-40 v2 diagnostic is not deployed" >&2
    exit 3
  }
  OUT_DIR=$OUT PREFIX=slot40-v2-arm TIMEOUT_SEC=$TIMEOUT \
    scripts/hw-jtag-repl.sh --verified-input --no-readback \
      --form "$(jq -r '.test.form' "$DEPLOY")"
  sleep 2
  capture_screen slot40-v2-screen
  grep -q '(%disk-load-lib 39 1)' "$OUT/slot40-v2-screen.txt" || {
    echo "Slot-40 v2 diagnostic form was not delivered" >&2
    exit 3
  }
  if grep -q '\*\*\* vm: bad bytecode' "$OUT/slot40-v2-screen.txt"; then
    echo "Slot-40 failed outside the instrumented publication edges."
  elif grep -Eq '(^|[[:space:]])t([[:space:]]|$)' \
      "$OUT/slot40-v2-screen.txt"; then
    echo "Slot-40 completed; no publication error reproduced."
  else
    echo "Slot-40 v2 error hold reached; capture PC next."
  fi
  ;;
capture)
  [ -e "$OUT/hardware-run.started" ] || {
  ...
  python3 "$PC"
  capture_screen slot40-v2-screen
  echo "Slot-40 v2 failure PC captured; diagnostic retired."
  ;;
esac
