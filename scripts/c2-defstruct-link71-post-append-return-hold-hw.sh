#!/bin/sh
# Pristine Link 71 plus a late three-byte hold after append_staged returns.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-35}
OUT=build/post-promotion/link71-defstruct-header-crc-domain/post-append-return-hold-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_defstruct_link71_post_append_return_hold.py
M65=$TOOLS/m65
...
deploy)
  [ ! -e "$OUT/hardware-run.started" ] || {
    echo "post-append diagnostic is one-shot" >&2
    exit 3
  }
  ...
  readback "$((C2J_START))" "$C2J_BYTES" "$OUT/deploy-readback-zero-c2j.bin"
  cmp "$ZERO_C2J" "$OUT/deploy-readback-zero-c2j.bin"
  run_m65 -r -1 "$PRG"
  poll=0
  while [ "$poll" -lt 45 ]; do
    capture_screen boot
  ...
  echo "Pristine Link 71 ready; post-append hold installed late."
  ;;
arm)
  [ -e "$OUT/hardware-run.started" ] || exit 3
  OUT_DIR=$OUT PREFIX=post-append-arm TIMEOUT_SEC=$TIMEOUT \
...
if [ "$ACTION" = arm ] || [ "$ACTION" = capture ]; then
  ...
  while [ "$i" -le 3 ]; do
    dir=$OUT/capture-$i
    mkdir "$dir"
    readback 0x00000000 128 "$dir/zero-page.bin"
    readback 0x0000b9a0 96 "$dir/vm-state-low.bin"
    readback 0x0000bfa0 75 "$dir/vm-code-and-state.bin"
    readback 0x0000c000 1024 "$dir/software-stack-window.bin"
    readback "$((C2J_START))" "$C2J_BYTES" "$dir/c2j.bin"
    [ "$i" -eq 3 ] || sleep 1
    i=$((i + 1))
  done
  for name in zero-page vm-state-low vm-code-and-state software-stack-window c2j; do
    cmp "$OUT/capture-1/$name.bin" "$OUT/capture-2/$name.bin"
    cmp "$OUT/capture-1/$name.bin" "$OUT/capture-3/$name.bin"
  done
  capture_screen post-append-hold
  echo "Post-append return state captured; diagnostic retired."
fi
