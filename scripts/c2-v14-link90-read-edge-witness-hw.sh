#!/bin/sh
# One-contact non-promotable Link-90 refill/read/return witness.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v14_link90_read_edge_witness.py
BASE=build/post-promotion/v14/link90-read-edge-witness
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "read-edge hardware contact already consumed" >&2; exit 3;
}
mkdir -p "$OUT"
...
# The autonomous target path terminates immediately and the diagnostic Runtime
# holds its terminal state.  Keep monitor traffic outside execution.
sleep 10
...
[ "$state" = 227 ] || {
  echo "read-edge identity did not reach VM error state: $state" >&2; exit 1;
}

witness_address=$(($(jq -r '.witness.address' "$DEPLOY")))
witness_bytes=$(jq -r '.witness.bytes' "$DEPLOY")
readback "$witness_address" "$witness_bytes" "$OUT/read_edge_witness.bin"
