#!/bin/sh
# One-contact non-promotable Link-90 production-order witness.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v14_link90_production_order_witness.py
BASE=build/post-promotion/v14/link90-production-order-witness
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "production-order hardware contact already consumed" >&2; exit 3;
}
mkdir -p "$OUT"
...
# The diagnostic path is host-proven to terminate immediately.  Keep all
# monitor traffic outside execution: one bounded quiet interval, then one
# terminal-state read and the commissioned post-stop witness read.
sleep 10
readback "$state_address" 1 "$OUT/lisp65_runtime_state.bin"
state=$(od -An -tu1 "$OUT/lisp65_runtime_state.bin" | tr -d ' ')
case "$state" in
  3|227) ;;
  *) echo "witness identity did not reach a terminal state: $state" >&2; exit 1 ;;
esac

address=$(($(jq -r '.witness.address' "$DEPLOY")))
bytes=$(jq -r '.witness.bytes' "$DEPLOY")
readback "$address" "$bytes" "$OUT/production_order_witness.bin"
