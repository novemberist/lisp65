#!/bin/sh
# One-shot cycle 2 of the Link-64 C2D-reader bounds-reject hold.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-30}
WAIT=${WAIT:-28}
OUT=build/c2.2/hardware-link64-c2d-reader-bounds-hold-cycle2-NONPROMOTABLE
...
PREPARE=tools/host-lisp/c2_link64_c2d_reader_bounds_hold_cycle2.py
M65=$TOOLS/m65
...
[ ! -e "$OUT/hardware-run.started" ] || {
  echo "reader-bounds cycle 2 is one-shot" >&2
  exit 3
}
...
if "lisp65>" not in text or "*** vm:" in text:
    raise SystemExit("reader-bounds cycle 2 did not reach a clean REPL")
PY
...
echo "Cycle 2 armed. Press physical RETURN; report hang or visible output."
