#!/bin/sh
# Read-only postmortem for cycle 2 of the Link-64 reader-bounds hold.
set -eu
cd "$(dirname "$0")/.."
...
[ -e "$OUT/hardware-run.started" ] || {
  echo "reader-bounds cycle 2 was not deployed" >&2
  exit 3
}
[ ! -e "$OUT/capture-1" ] || {
  echo "reader-bounds cycle 2 was already captured" >&2
  exit 3
}
...
(out / "capture-times.json").write_text(json.dumps({
    "format": "lisp65-Link64-c2d-reader-bounds-nohit-times-v2",
    "interval_seconds": [0, 1, 5],
    "captures": rows,
}, indent=2, sort_keys=True) + "\n")
PY
screen_text reader-bounds-screen
echo "reader-bounds cycle-2 First Red captured; evaluation pending."
