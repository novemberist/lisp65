#!/bin/sh
# Boot pristine Link 71, activate Slot-40 clear holds late, and run one load.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-35}
OUT=build/post-promotion/link71-defstruct-header-crc-domain/slot40-clear-hold-v6-late-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_defstruct_link71_slot40_clear_hold_v6.py
PC=tools/host-lisp/c2_defstruct_link71_slot40_clear_pc_capture_v6.py
M65=$TOOLS/m65
...
deploy)
  [ ! -e "$OUT/hardware-run.started" ] || {
    echo "Link-71 Slot-40 clear diagnostic is one-shot" >&2
    exit 3
  }
  ...
  echo "Pristine Link 71 ready; install late clear carrier next."
  ;;
install)
  [ -e "$OUT/hardware-run.started" ] || exit 3
  [ ! -e "$OUT/late-carrier.installed" ] || {
    echo "late clear carrier installation is one-shot" >&2
    exit 3
  }
  ...
  echo "Identity-preserving Slot-40 clear carrier installed after boot."
  ;;
arm)
  [ -e "$OUT/late-carrier.installed" ] || exit 3
  OUT_DIR=$OUT PREFIX=slot40-clear-v6-arm TIMEOUT_SEC=$TIMEOUT \
    scripts/hw-jtag-repl.sh --verified-input --no-readback \
      --form "$(jq -r '.test.form' "$DEPLOY")"
  sleep 2
  capture_screen slot40-clear-v6-screen
  grep -q '(%disk-load-lib 39 1)' "$OUT/slot40-clear-v6-screen.txt" || exit 3
  if grep -q '\*\*\* vm: bad bytecode' \
      "$OUT/slot40-clear-v6-screen.txt"; then
    echo "Slot-40 publish and clear passed; final Slot-39 remains."
  elif grep -Eq '(^|[[:space:]])t([[:space:]]|$)' \
      "$OUT/slot40-clear-v6-screen.txt"; then
    echo "Load completed; no post-publish error reproduced."
  else
    echo "Slot-40 clear hold reached; capture PC and state next."
  fi
  ;;
capture)
  [ -e "$OUT/late-carrier.installed" ] || exit 3
  ...
  capture_screen slot40-clear-v6-screen
  echo "Slot-40 clear PC/state captured; diagnostic retired."
  ;;
esac
