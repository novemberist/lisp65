#!/bin/sh
# One-row physical progress witness for the hook-armed Link-82 diagnostic.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v16_boot_progress_witness.py
DEPLOY=build/c2.3/v1.6-defstruct-phase-c/deployment.json
OUT=${OUT:-build/c2.3/v1.6-defstruct-closing-session/d2-progress-witness-appointment}
...
if [ "$ACTION" = dry-run ]; then
  python3 "$PY" check
  python3 "$PY" selftest
  echo "D2 PROGRESS DRY RUN PASS samples=3 spacing=2s forms=0"
  exit 0
fi
...
if [ "$ACTION" = capture ]; then
  [ -e "$OUT/stage.ready" ] || {
    echo "progress appointment is not staged" >&2; exit 3;
  }
  [ ! -e "$OUT/capture.consumed" ] || {
    echo "progress capture already consumed" >&2; exit 3;
  }
...
[ ! -e "$OUT/stage.consumed" ] || {
  echo "progress stage already consumed" >&2; exit 3;
}
: > "$OUT/stage.consumed"
run_m65 -F
sleep 5
...
run_m65 -H "$product"
payload_bytes=$(($(jq -r '.diagnostic.prg.bytes' "$DEPLOY") - 2))
...
PY
: > "$OUT/stage.ready"
echo "D2 PROGRESS STAGE READY: type RUN and press RETURN on the physical keyboard."
