#!/bin/sh
# Fresh Link 107 D1. Never use FTP against the live product.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
CONFIG=config/c2-v150-v21-dependent-vma-far-device-session.json
PY=tools/host-lisp/c2_v21_dependent_vma_d1.py
OUT=${OUT:-build/c2.3/v2.1-dependent-vma-d1}
...
PRODUCT_REMOTE=V21L107.D81
LIBRARY_REMOTE=V21L107L.D81
...
if [ "$ACTION" = dry-run ]; then
  echo "D1 ONLY: fresh BASIC -> one FTP lifetime/readback -> Link 107 product mount"
...
if [ "$ACTION" = stage ]; then
  [ ! -e "$OUT/contact.consumed" ] || {
    echo "Link 107 D1 contact already consumed" >&2; exit 3;
  }
...
[ -e "$OUT/signs-awaiting-owner-confirmation" ] \
  ...
  && [ ! -e "$OUT/owner-visible-signs-confirmed" ] || {
    echo "Link 107 D1 owner-confirmation state invalid" >&2; exit 3;
  }
printf '%s\n' \
