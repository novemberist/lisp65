#!/bin/sh
# Owner-authorized physical launch with control-shaped pre-owner choreography.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v16_control_shaped_discriminator.py
DEPLOY=build/c2.3/v1.6-defstruct-phase-c/deployment.json
OUT=${OUT:-build/c2.3/v1.6-defstruct-closing-session/d2-control-shaped-discriminator}
PRODUCT=build/c2.3/v1.6-defstruct-closing-session/d2-corrected-view-quiet-appointment/diagnostic-link82-corrected-view-b5c3.prg
...
if [ "$ACTION" = dry-run ]; then
  python3 "$PY" check
  python3 "$PY" selftest
  echo "D2 CONTROL-SHAPED DRY RUN PASS no-prelaunch-monitor first-t1-min=27.653s"
  exit 0
fi
...
if [ "$ACTION" = capture ]; then
  ...
  [ ! -e "$OUT/capture.consumed" ] || { echo "capture consumed" >&2; exit 3; }
  : > "$OUT/capture.consumed"
  exec python3 "$PY" capture --device "$DEVICE"
fi

[ ! -e "$OUT/stage.consumed" ] || { echo "stage consumed" >&2; exit 3; }
: > "$OUT/stage.consumed"
run_m65 -F
sleep 5
screen fresh-basic
...
PY
: > "$OUT/stage.ready"
echo "D2 CONTROL-SHAPED STAGE READY: type RUN and press RETURN physically."
