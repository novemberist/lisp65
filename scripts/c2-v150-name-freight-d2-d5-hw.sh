#!/bin/sh
# Continue a green v1.5 name-freight D1 through D2-D5.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
ROWS=config/c2-v150-link97-device-rows.json
PY=tools/host-lisp/c2_v150_name_freight_d2_d5.py
D1=tests/bytecode/dialect-v2/evidence/architecture-blocks/c2.3-v1.5.0-name-freight-d1-receipt.json
OUT=${OUT:-build/c2.3/v1.5.0-name-freight-d2-d5}
TOOLS=${TOOLS:-tools/m65tools}
DEVICE=${DEVICE:-/dev/ttyUSB1}
M65=$TOOLS/m65
TIMEOUT=${TIMEOUT:-60}
# NO-POST-BOOT-FTP: the library was uploaded before D1 boot.
...
if [ "$ACTION" = dry-run ]; then
  ...
  echo "OWNER: Freezer-mount V15NFLIB.D81 on drive 8 and return with F3"
  echo "D2-D5: 19 physical forms; one postcondition after each quiet floor"
  echo "FINAL: guard plus physical Bank-0 nsym/npool headroom readback"
  exit 0
fi
...
if [ "$ACTION" = confirm-library ]; then
  [ ! -e "$OUT/library-owner-confirmed" ] || {
    echo "v1.5 name-freight library already confirmed" >&2; exit 3;
  }
  D1_GREEN=$(jq -r '.status' "$D1")
  [ "$D1_GREEN" = V150-NAME-FREIGHT-D1-GREEN ] || {
    echo "green name-freight D1 authority absent" >&2; exit 3;
  }
...
if [ "$ACTION" = wait-row ]; then
  [ -e "$OUT/library-owner-confirmed" ] && [ -e "$OUT/next-row" ] || {
    echo "v1.5 name-freight row is not armed" >&2; exit 3;
  }
...
[ -e "$OUT/rows-complete" ] && [ ! -e "$OUT/final-capture-complete" ] || {
  echo "v1.5 name-freight final-capture state invalid" >&2; exit 3;
}
run_m65 -H --memsave \
...
python3 "$PY" record
echo "V1.5 NAME-FREIGHT D1-D5 GREEN; CPU STOPPED; OWNER HALT PENDING."
