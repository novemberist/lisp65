#!/bin/sh
# Fresh Link 106 D1. Never use FTP against the live product.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
CONFIG=config/c2-v150-v20-phase02b-header-consumption-far-device-session.json
PY=tools/host-lisp/c2_v20_phase02b_header_consumption_d1.py
OUT=${OUT:-build/c2.3/v2.0-phase02b-header-consumption-d1}
...
PRODUCT_REMOTE=V20L106.D81
LIBRARY_REMOTE=V20L106L.D81
...
if [ "$ACTION" = dry-run ]; then
  echo "D1 ONLY: fresh BASIC -> one FTP lifetime/readback -> Link 106 product mount"
...
if [ "$ACTION" = stage ]; then
  [ ! -e "$OUT/contact.consumed" ] || {
    echo "Link 106 D1 contact already consumed" >&2; exit 3;
  }
...
[ -e "$OUT/signs-awaiting-owner-confirmation" ] \
  ...
  && [ ! -e "$OUT/owner-visible-signs-confirmed" ] || {
    echo "Link 106 D1 owner-confirmation state invalid" >&2; exit 3;
  }
printf '%s\n' \
