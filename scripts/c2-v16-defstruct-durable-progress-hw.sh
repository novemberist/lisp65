#!/bin/sh
# One physical RUN and one durable Link-82 boot-progress row.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v16_boot_durable_progress.py
DEPLOY=build/c2.3/v1.6-defstruct-phase-c/deployment.json
OUT=${OUT:-build/c2.3/v1.6-defstruct-closing-session/d2-durable-progress-appointment}
PRODUCT=$OUT/diagnostic-link82-durable-b5c3.prg
...
if [ "$ACTION" = dry-run ]; then
  python3 "$PY" check
  python3 "$PY" selftest
  echo "D2 DURABLE PROGRESS DRY RUN PASS samples=3 spacing=5s forms=0"
  exit 0
fi
...
if [ "$ACTION" = capture ]; then
  [ -e "$OUT/stage.ready" ] || {
    echo "durable progress appointment is not staged" >&2; exit 3;
  }
  [ ! -e "$OUT/capture.consumed" ] || {
    echo "durable progress capture already consumed" >&2; exit 3;
  }
...
[ ! -e "$OUT/stage.consumed" ] || {
  echo "durable progress stage already consumed" >&2; exit 3;
}
: > "$OUT/stage.consumed"
run_m65 -F
sleep 5
...
readback 0x0000b5c3 1 "$OUT/witness-before-resume.bin"
cmp "$SENTINEL" "$OUT/witness-before-resume.bin"
run_m65 -r
sleep 3
screen launch-ready
...
PY
readback 0x0000b5c3 1 "$OUT/witness-before-run.bin"
cmp "$SENTINEL" "$OUT/witness-before-run.bin"
run_m65 -r
: > "$OUT/stage.ready"
echo "D2 DURABLE PROGRESS STAGE READY: type RUN and press RETURN physically."
