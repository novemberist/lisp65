#!/bin/sh
# One-shot nonpromotable Link-62 Slot-39 64-frame threshold hold.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-30}
WAIT=${WAIT:-28}
OUT=build/c2.2/hardware-link62-slot39-threshold-hold-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_link62_slot39_threshold_hold.py
M65=$TOOLS/m65
...
if "lisp65>" not in text or "*** vm:" in text:
    raise SystemExit("threshold-hold boot did not reach a clean REPL")
PY
...
(out / "capture-times.json").write_text(json.dumps({
    "format": "lisp65-Link62-slot39-threshold-hold-times-v1",
...
python3 "$PY" evaluate
echo "Threshold-hold captured and diagnostic identity retired."
