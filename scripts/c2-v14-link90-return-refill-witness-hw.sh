#!/bin/sh
# One-contact non-promotable Link-90 helper-return/refill witness.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v14_link90_return_refill_witness.py
BASE=build/post-promotion/v14/link90-return-refill-witness
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "return/refill hardware contact already consumed" >&2; exit 3;
}
mkdir -p "$OUT"
...
# The autonomous path is allowed to finish before the only two RAM reads.
sleep 10
...
[ "$state" = 227 ] || {
  echo "return/refill identity did not reach VM error state: $state" >&2; exit 1;
}

witness_address=$(($(jq -r '.witness.address' "$DEPLOY")))
witness_bytes=$(jq -r '.witness.bytes' "$DEPLOY")
readback "$witness_address" "$witness_bytes" "$OUT/return_refill_witness.bin"
