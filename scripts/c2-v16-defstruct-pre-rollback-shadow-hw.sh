#!/bin/sh
# Owner-authorized, one-shot pre-rollback-shadow measured-form repeat.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v16_pre_rollback_shadow_contact.py
DEPLOY=build/c2.3/v1.6-defstruct-closing-session/d2-pre-rollback-shadow-repeat/authorized-deployment.json
OUT=${OUT:-build/c2.3/v1.6-defstruct-closing-session/d2-pre-rollback-shadow-repeat}
...
if [ "$ACTION" = dry-run ]; then
  python3 "$PY" check
  python3 "$PY" selftest
  echo "PRE-ROLLBACK SHADOW DRY RUN PASS reset-domain=50816 C2J=CLEAR shadow=C06B physical-input=3 quiet=180s stops=1"
  exit 0
fi
...
if [ "$ACTION" = stage ]; then
  [ ! -e "$OUT/stage.consumed" ] || { echo "shadow-repeat stage already consumed" >&2; exit 3; }
  : > "$OUT/stage.consumed"
  run_m65 -F
  sleep 5
  screen fresh-basic
...
PY
  ...
  run_m65 -r
  echo "PRE-ROLLBACK SHADOW STAGE READY: type RUN and press RETURN physically."
  exit 0
fi

if [ "$ACTION" = verify-boot ]; then
  [ -e "$OUT/stage.ready" ] || { echo "shadow-repeat stage absent" >&2; exit 3; }
...
if [ "$ACTION" = arm ]; then
  ...
  [ ! -e "$OUT/arm.consumed" ] || { echo "shadow record arm already consumed" >&2; exit 3; }
  : > "$OUT/arm.consumed"
...
PY
  ...
  : > "$OUT/arm.ready"
  run_m65 -r
  echo "SHADOW R/A/I/G RECORD ARMED: type (defstruct point x y) physically, then immediately return here."
  exit 0
fi
