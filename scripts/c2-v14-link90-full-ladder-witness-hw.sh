#!/bin/sh
# One-contact non-promotable Link-90 full remaining-edge ladder.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
PY=tools/host-lisp/c2_v14_link90_full_ladder_witness.py
BASE=build/post-promotion/v14/link90-full-ladder-witness
...
[ ! -e "$OUT/contact.consumed" ] || {
  echo "full-ladder hardware contact already consumed" >&2; exit 3;
}
mkdir -p "$OUT"
...
[ "$state" = 227 ] || {
  echo "full-ladder identity did not reach VM error state: $state" >&2; exit 1;
}

witness_address=$(($(jq -r '.witness.address' "$DEPLOY")))
witness_bytes=$(jq -r '.witness.bytes' "$DEPLOY")
readback "$witness_address" "$witness_bytes" "$OUT/full_ladder_witness.bin"
