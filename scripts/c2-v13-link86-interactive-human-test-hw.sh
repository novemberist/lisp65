#!/bin/sh
# Final physical-keyboard acceptance for the Link-86 interactive Ship sample.
set -eu
cd "$(dirname "$0")/.."

ACTION=${1:-}
CONFIG=config/c2-ship-builder-v1-link86-interactive-human-test.json
OUT=build/ship-builder/v13/link86-interactive-human-test/run
...
if [ "$ACTION" = dry-run ]; then
  [ -f "$image" ]
  echo "DRY-RUN: cold reset -> fresh BASIC -> exact committed Link-86 D81"
  echo "ASSERT: Runtime state 2 proves arm/readback and a live jiffy"
