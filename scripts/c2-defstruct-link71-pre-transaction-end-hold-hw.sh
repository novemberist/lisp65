#!/bin/sh
# Pristine Link 71 plus a late hold before transaction_end.
set -eu
cd "$(dirname "$0")/.."
...
TIMEOUT=${TIMEOUT:-35}
OUT=build/post-promotion/link71-defstruct-header-crc-domain/pre-transaction-end-hold-NONPROMOTABLE
DEPLOY=$OUT/deployment.json
PY=tools/host-lisp/c2_defstruct_link71_pre_transaction_end_hold.py
M65=$TOOLS/m65
...
deploy)
  ...
  echo "Pristine Link 71 ready; pre-transaction hold installed late."
  ;;
arm)
  OUT_DIR=$OUT PREFIX=pre-transaction-arm TIMEOUT_SEC=$TIMEOUT \
...
if [ "$ACTION" = arm ] || [ "$ACTION" = capture ]; then
  ...
  capture_screen pre-transaction-hold
  echo "Pre-transaction-end state captured; diagnostic retired."
fi
