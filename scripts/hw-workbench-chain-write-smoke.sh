#!/bin/sh
# Reproducible HW smoke for the M3 two-sector chain-write step.
#
# This test writes only to a generated throwaway copy of the Workbench D81. It
# runs a dedicated PRG that writes a two-sector source chain and allocates the
# sectors in the BAM, downloads the D81, verifies the exact host diff, then
# boots the Workbench against that throwaway image and loads the chain via
# %disk-load-file. No hard JTAG reset is used.
set -eu
...
prefix="${PREFIX:-hw-workbench-chain-write}"
wait_sec="${WAIT_SEC:-3}"
...
remote_d81="${WORKBENCH_M3_REMOTE_D81:-L65M3.D81}"
ship_d81="${MVP_VM_SHIP_D81:-build/ship/lisp65-mvp-workbench.d81}"
before_d81="${WORKBENCH_M3_BEFORE_D81:-build/hw/workbench-m3-before.d81}"
after_d81="${WORKBENCH_M3_AFTER_D81:-build/hw/workbench-m3-after.d81}"
prg="${M65HWCHAINWRITEPRG:-build/lisp65-mega65-hw-chain-write-smoke.prg}"
source_lisp="${WORKBENCH_M3_SOURCE:-tests/disk/m3-chain-source.lisp}"
...
usage: $0 [options]
  ...
  --source <file>       expected Lisp chain payload (default: $source_lisp)
  ...
  --prg <file>          chain-write PRG (default: $prg)
  --no-restore          leave machine after the M3 oracle/readback path
...
if [ "$build" = "1" ]; then
  echo "==> baue MVP-Ship und Chain-Write-Test-PRG"
  make mvp-ship hw-workbench-chain-write-smoke-prg
fi
...
restore_workbench() {
  ...
  [ "$restored" = "0" ] || return 0
  restored=1
  echo "==> stelle Workbench nach M3-Smoke wieder her"
...
if [ "$dry_run" = "1" ]; then
  ...
  echo "DRY-RUN: python3 tools/host-lisp/d81_chain_write_diff.py --selftest $before_d81 --source $source_lisp --track $target_track --first-sector $first_sector --second-sector $second_sector"
else
  [ -f "$ship_d81" ] || { echo "Fehler: Workbench-D81 fehlt: $ship_d81" >&2; exit 3; }
  [ -f "$prg" ] || { echo "Fehler: Chain-Write-PRG fehlt: $prg" >&2; exit 3; }
  [ -f "$source_lisp" ] || { echo "Fehler: M3-Quelle fehlt: $source_lisp" >&2; exit 3; }
  cp "$ship_d81" "$before_d81"
  rm -f "$after_d81"
  python3 tools/host-lisp/d81_chain_write_diff.py --selftest "$before_d81" \
    --source "$source_lisp" --track "$target_track" \
    --first-sector "$first_sector" --second-sector "$second_sector"
fi
...
upload_throwaway() {
  ...
  echo "==> lege M3-Wegwerf-D81 auf die SD: $remote_d81"
  if [ "$dry_run" = "1" ]; then
    if [ -n "$ip" ]; then
...
run_prg() {
  ...
  [ "$dry_run" = "1" ] && set -- "$@" --dry-run
  set -- "$@" "$prg"
  echo "==> starte M3-Chain-Write-PRG (kein m65 -F)"
...
verify_after_image() {
  if [ "$dry_run" = "1" ]; then
    echo "DRY-RUN: python3 tools/host-lisp/d81_chain_write_diff.py $before_d81 $after_d81 --source $source_lisp --track $target_track --first-sector $first_sector --second-sector $second_sector"
  else
    python3 tools/host-lisp/d81_chain_write_diff.py "$before_d81" "$after_d81" \
      --source "$source_lisp" --track "$target_track" \
      --first-sector "$first_sector" --second-sector "$second_sector"
  fi
}
...
deploy_oracle_workbench() {
  ...
  echo "==> deploye Workbench fuer M3-Load-Oracle gegen $remote_d81"
  if [ "$dry_run" = "1" ]; then
...
deploy_oracle_workbench
run_oracle_phase load-chain "\"m3-load-ok\"" "(if (%disk-load-file $target_track $first_sector) \"m3-load-ok\" \"m3-load-fail\")"
run_oracle_phase run-chain "737" "(m3-chain-run)"

restore_armed=0
restore_workbench

echo "PASS Workbench chain-write HW smoke"
