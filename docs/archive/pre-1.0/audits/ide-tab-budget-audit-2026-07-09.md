...
- **Kandidat A** war der empfohlene direkte Nachzug: die vier Defuns aus
  `lib/ide-ui.lisp` entfernen und in denselben Commit die zugehoerigen
...
Kandidat A wurde nach Review umgesetzt: Die vier `ide-runtime-symbol-*`-
Funktionen wurden aus `lib/ide-ui.lisp` entfernt und die gekoppelten
Bytecode-Subset-/Disk-Lib-Listen bereinigt. Ergebnis der Gates:

```text
bytecode-p0-ide-lib-check: PASS functions=180 cases=70 objects=250
  code_bytes=14076 dir_bytes=1750
...
Kandidat B (`ide-delete-line`, `ide-insert-line`) wurde nach separatem
Claude-Audit umgesetzt; siehe `docs/ide-line-op-audit-2026-07-09.md`.

Nachtrag: Der spaetere Directory-RETURN-Slice nutzt zwei der vier freigewordenen
Slots. Candidate B holt diese zwei Slots wieder zurueck. Aktueller Pin:
`disk_lib=180`, `load_used=500/512`, Raw-Headroom `12`,
Post-Align-Headroom `8`; siehe `docs/workbench-gate.md`.
