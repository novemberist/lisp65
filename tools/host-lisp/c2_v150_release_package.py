#!/usr/bin/env python3
"""Prepare and twice verify the four immutable v1.5.0 Halt-#2 assets."""
...
ROLE_PATHS = {
    ...
    "library-place": "libraries/place.l65s",
    "library-defstruct": "libraries/defstruct.l65s",
}

DOCUMENTS = {
    "docs/release-notes.md": ROOT / "docs/releases/1.5.0.md",
...
PROOFS = {
    ...
    "proof/boot-duration.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v1.5.0-boot-duration-device-receipt.json"),
    "proof/halt-1-owner.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v1.5.0-halt-1-owner-decision-receipt.json"),
}
...
def artifact_sources(product_root: Path) -> dict[str, dict[str, Any]]:
    authority = load(AUTHORITY, "v1.5 public authority")
    ...
    for role, row in rows.items():
        ...
        require(
            ...
            f"selected v1.5 product byte drift: {role}")
        row["source"] = source
    return rows
...
def validate_authorities(product_root: Path, clean_receipt: Path,
    ...
    authority = load(AUTHORITY, "v1.5 public authority")
    clean = load(clean_receipt, "v1.5 clean-build receipt")
    ...
    require(
        ...
        and authority.get("selected_variant") == "v1.5"
        and authority.get("artifact_count") == 25
        ...
        and clean.get("artifact_count") == 25
        and clean.get("selected_variant") == "v1.5"
        ...
        and clean.get("artifact_set_sha256")
            == authority["sealed_product_artifact_set_sha256"],
        "v1.5 public authority/clean-build mismatch")
...
def readme(product_set: str) -> bytes:
    return (
        "LISP65 WORKBENCH 1.5.0\n"
        "========================\n\n"
        "This is the Halt-#1-selected v1.5 release package.\n"
        ...
        "  media/lisp65-library.d81  optional v1.5 libraries\n\n"
        "Optional packages:\n"
        "  (require 'string-extra)   capitalize, string-split\n"
        "  (require 'inspect)        who-calls, trace, untrace\n"
        "  (require 'defstruct)      positional structures (loads place)\n\n"
...
def fail(message):
    print(f"lisp65 1.5.0 offline verification: FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
if not (value.get("format") == "lisp65-v1.5.0-release-package-v1"
...
if not (isinstance(rows, list) and len(rows) == 25
        and product.get("artifact_count") == 25): fail("25-role inventory missing")
roles = set(); paths = set()
for row in rows:
...
if len(roles) != 25 or len(paths) != 25: fail("duplicate role or ship path")
...
if not (claims.get("trace") == "hardware-green-exact-BCODE-restoration"
        and claims.get("defstruct") == "hardware-green-positional-option-free"
        and claims.get("line_atomic_input") == "not-claimed"):
    fail("claim boundary drift")
print(f"lisp65 1.5.0 offline verification: PASS roles=25 files={len(files)} "
...
def build_product(root: Path, source_commit: str, clean: dict[str, Any],
    ...
    manifest = {
        "format": "lisp65-v1.5.0-release-package-v1",
        "version": 1, "release": RELEASE,
        "status": "prepared-awaiting-owner-publication",
        "release_authorized": False, "selected_variant": "v1.5",
        ...
        "product": {
            ...
            "user_headroom": {"symbol_slots": 34, "namepool_bytes": 545},
        ...
        "claims": {
            "banner": "WORKBENCH 1.5.0-on-physical-MEGA65",
            "surface": ["who-calls", "trace", "untrace", "capitalize",
                        "string-split", "defstruct"],
            "trace": "hardware-green-exact-BCODE-restoration",
            "defstruct": "hardware-green-positional-option-free",
            "boot_seconds": {"v1.4.0": 31, "v1.5.0": 36},
...
def prepare(source_repository: Path, source_commit: str, product_root: Path,
    ...
    release_root = ROOT / "build/release-v1.5.0"
    ...
    require(manifest_a == manifest_b, "v1.5 varied product staging differs")
    ...
    require(product_a.read_bytes() == product_b.read_bytes(),
            "v1.5 product double-pack differs")
    ...
    require(source_a.read_bytes() == source_b.read_bytes(),
            "v1.5 source double-pack differs")
    ...
    value = {
        "format": "lisp65-v150-release-package-preparation-v1",
    ...
    receipt = release_root / "v1.5.0-package-preparation-receipt.json"
...
def check(publish: Path) -> dict[str, Any]:
    ...
    require(publish.is_dir()
            ...
            "v1.5 publish directory is not the exact four-asset set")
...
def selftest() -> None:
    ...
    for key, value in (("path", "c"), ("bytes", 3),
        ...
        require(COMMON.package_set_sha(candidate) != baseline,
                f"v1.5 package mutation survived: {key}")
    with tempfile.TemporaryDirectory(prefix="lisp65-v150-tar-selftest-") as raw:
        ...
        require(first.read_bytes() == second.read_bytes(),
                "v1.5 deterministic archive selftest failed")
    print("c2-v150-release-package: SELFTEST PASS mutations=4 double-pack=1")
...
def main() -> int:
    ...
    prepare_parser.add_argument("--output", type=Path,
                                default=ROOT / "build/release-v1.5.0/publish")
    ...
    try:
        if args.action == "selftest":
            ...
            result = prepare(args.source_repository, args.source_commit,
                             ...
                             args.output.resolve())
            print("c2-v150-release-package: PREPARED "
                  f"assets=4 roles=25 source={result['source_commit']} "
                  f"set={result['product']['artifact_set_sha256']}")
        else:
            result = check(args.publish.resolve())
            print("c2-v150-release-package: CHECK PASS "
                  f"assets=4 roles=25 source={result['source_commit']}")
        return 0
    except (PackageError, COMMON.PackageError, OSError, KeyError, ValueError,
            json.JSONDecodeError) as error:
        print(f"c2-v150-release-package: FAIL: {error}", file=sys.stderr)
        return 1
