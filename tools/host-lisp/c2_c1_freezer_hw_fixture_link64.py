#!/usr/bin/env python3
"""Run C1 Cutpoints 3/4 on the immutable Link-64 product identity."""
...
import c2_c1_freezer_carrier_link64 as CARRIER  # noqa: E402
...
OUT = ROOT / (
    "build/c2.2/"
    "c1-freezer-hardware-link64-cutpoints3-4-NONPROMOTABLE")
HARDWARE_RECEIPT = CARRIER.EVIDENCE / (
    "c2.2-link64-C1-Freezer-four-cutpoint-hardware-receipt.json")
DEPLOYMENT_STATUS = "ready-nonpromotable-Link64-cutpoints-3-and-4"
CARRIER_STATUS = "passed-Link64-capacity-and-gates-awaiting-hardware"
CARRIER_BASENAME = (
...
def current_authority() -> dict[str, Path]:
    ...
    for name, path in paths.items():
        M.require(path.is_file(), f"missing Link-64 {name}: {path}")
    ...
    M.require(
        ...
        and replay["gates"]["stateless_completion_length_ELF"]
        ...
        and carrier["construction"]["main_family_crc16"] == "0x472a"
        and carrier["construction"]["region1_byteidentical_Link64"]
        and carrier["proof"]["post_RTS_tail"]["bytes"] == 2
        and carrier["proof"]["post_shelf_region1"][
            "durable_source"] == "0x08300000"
        and carrier["proof"]["completion_retry_length"][
        ...
        and boot_manifest["storage"]["crc16"] == 0x0F92
        ...
        "Link-64 C1 fixture authority is incomplete")
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-c1-freezer-hw-fixture-link64: FAIL " + str(error),
            file=sys.stderr)
        raise SystemExit(2)
