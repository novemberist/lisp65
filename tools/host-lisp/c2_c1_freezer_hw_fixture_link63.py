#!/usr/bin/env python3
"""Run C1 Cutpoints 3/4 on the immutable Link-63 product identity."""
...
import c2_c1_freezer_carrier_link63 as CARRIER  # noqa: E402
...
OUT = ROOT / (
    "build/c2.2/"
    "c1-freezer-hardware-link63-cutpoints3-4-NONPROMOTABLE")
HARDWARE_RECEIPT = CARRIER.EVIDENCE / (
    "c2.2-link63-C1-Freezer-four-cutpoint-hardware-receipt.json")
DEPLOYMENT_STATUS = "ready-nonpromotable-Link63-cutpoints-3-and-4"
CARRIER_STATUS = "passed-Link63-capacity-and-gates-awaiting-hardware"
CARRIER_BASENAME = (
    "runtime-overlays-session-c1-freezer-link63-stage-bound.bin")
CARRIER_REGION1_BASENAME = (
    "runtime-overlays-session-c1-freezer-link63-region1.bin")
...
def current_authority() -> dict[str, Path]:
    ...
    for name, path in paths.items():
        M.require(path.is_file(), f"missing Link-63 {name}: {path}")
    ...
    M.require(
        ...
        and replay["gates"]["completion_retry_length_ELF"]
        ...
        and carrier["construction"]["main_family_crc16"] == "0xdfe5"
        and carrier["construction"]["region1_byteidentical_Link63"]
        and carrier["proof"]["post_RTS_tail"]["bytes"] == 3
        and carrier["proof"]["post_shelf_region1"][
            "durable_source"] == "0x08300000"
        and carrier["proof"]["completion_retry_length"]["reloads"] == 2
        ...
        and boot_manifest["storage"]["crc16"] == 0xEDB7
        ...
        "Link-63 C1 fixture authority is incomplete")
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-c1-freezer-hw-fixture-link63: FAIL " + str(error),
            file=sys.stderr)
        raise SystemExit(2)
