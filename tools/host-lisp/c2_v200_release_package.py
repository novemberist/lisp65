#!/usr/bin/env python3
"""Prepare and twice verify the four immutable v2.0.0 Halt-B assets."""
...
SOURCE_EPOCH = 1788307200
AUTHORITY = ROOT / "config/c2-v200-public-build-authority.json"
DEFAULT_CLEAN_RECEIPT = ROOT / (
    "build/release-v2.0.0/v2.0.0-public-clean-build-receipt.json")
RELEASE_ROOT = ROOT / "build/release-v2.0.0"
PREPARATION_RECEIPT = RELEASE_ROOT / "v2.0.0-package-preparation-receipt.json"
PACKAGE_FORMAT = "lisp65-v2.0.0-release-package-v1"
PREPARATION_FORMAT = "lisp65-v200-release-package-preparation-v1"
BUNDLE_DOCS_VALIDATOR: Callable[[Path], dict[str, Any]] | None = None

COMMON.TOP = TOP
COMMON.SOURCE_EPOCH = SOURCE_EPOCH

ROLE_PATHS = {role: path for role, path in V160.ROLE_PATHS.items()
              if role not in {"optional-library-d81",
                              "optional-library-index", "library-v16core"}}
DOCUMENTS = {
    "docs/release-notes.md": ROOT / "docs/releases/2.0.0.md",
...
PROOFS = {
    "proof/final-device-session.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v2.0-release-strip-device-result-receipt.json"),
    "proof/device-attributions.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v2.0-release-device-attributions-receipt.json"),
    "proof/release-product-card.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v2.0.0-release-card-r3-receipt.json"),
    "proof/release-media.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v2.0.0-release-media-receipt.json"),
    "proof/candidate-seal.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v2.0.0-candidate-seal-receipt.json"),
}
...
def artifact_sources(product_root: Path) -> dict[str, dict[str, Any]]:
    authority = load(AUTHORITY, "v2.0 public authority")
    ...
    require(
        manifest.get("format") == "lisp65-v2.0-public-selected-product-v1"
        and manifest.get("status")
            == "passed-public-source-selected-v2.0-tier1-lossless-native-editor-stripped"
        ...
        "selected v2.0 product manifest drift")
    for role, row in rows.items():
        ...
        require(source.is_file() and not source.is_symlink()
                ...
                f"selected v2.0 product byte drift: {role}")
        row["source"] = source
    return rows
...
def validate_authorities(product_root: Path, clean_receipt: Path,
    ...
    authority = load(AUTHORITY, "v2.0 public authority")
    clean = load(clean_receipt, "v2.0 clean-build receipt")
    sources = artifact_sources(product_root)
    require(
        authority.get("format") == "lisp65-c2-public-build-authority-v8"
        and authority.get("release") == PRODUCT_RELEASE
        and authority.get("selected_variant") ==
            "v2.0-tier1-lossless-native-editor-stripped"
        and authority.get("artifact_count") == 19
        and clean.get("format")
            == "lisp65-c2-v200-public-clean-build-receipt-v1"
        and clean.get("status") == "passed"
        and clean.get("source_commit") == source_commit
        and clean.get("artifact_count") == 19
        and clean.get("selected_variant") ==
            "v2.0-tier1-lossless-native-editor-stripped"
        ...
        and clean.get("artifact_set_sha256")
            == authority["sealed_product_artifact_set_sha256"],
        "v2.0 public authority/clean-build mismatch")
...
def source_entries(repository: Path, source_commit: str,
                   clean: dict[str, Any]) -> list[tuple[str, int, bytes]]:
    """Archive the certified commit and add its derived public manifest."""
    result = subprocess.run(
        ["git", "ls-tree", "-rz", source_commit], cwd=repository,
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    require(result.returncode == 0, "cannot enumerate v2.0 public source commit")
    ...
    for raw in result.stdout.split(b"\0"):
        ...
        require(kind == "blob" and mode in ("100644", "100755"),
                f"unsupported v2.0 public source object: {encoded_path!r}")
        relative = encoded_path.decode("utf-8")
        COMMON.safe_relative(relative)
        require(relative != "PUBLIC-SOURCE-MANIFEST.json",
                "v2.0 source commit unexpectedly pins a derived manifest")
        blob = subprocess.run(
        ...
        rows.append({"path": relative, "bytes": len(blob.stdout),
                     "sha256": digest})
        entries.append((f"{TOP}/{relative}",
                        0o755 if mode == "100755" else 0o644,
                        blob.stdout))
    rows.sort(key=lambda row: row["path"])
    projection = {
        "format": "lisp65-public-source-projection-v1",
    ...
    require(
        projection["tree_sha256"] == clean["public_source_tree_sha256"]
        and hashlib.sha256(canonical(projection)).hexdigest()
            == clean["public_source_manifest_sha256"],
        "v2.0 source archive differs from the twice-built source projection")
    public_tree = hashlib.sha256()
    for row in rows:
        public_tree.update(row["path"].encode("utf-8"))
        public_tree.update(b"\0")
        public_tree.update(row["sha256"].encode("ascii"))
        public_tree.update(b"\n")
    manifest = {
        "format": "lisp65-public-source-manifest-v1",
        "source_commit": source_commit,
        "source_tree_clean": True,
        "file_count": len(rows),
        "tree_sha256": public_tree.hexdigest(),
        "files": rows,
    }
    entries.append((f"{TOP}/PUBLIC-SOURCE-MANIFEST.json", 0o644,
                    canonical(manifest)))
...
def readme(product_set: str) -> bytes:
    return (
        "LISP65 WORKBENCH 2.0.0\n"
        "========================\n\n"
        "This is the owner-Ship-selected v2.0 release package.\n"
        ...
        "The native lisp65> prompt owns the insertion-mode line editor.\n"
        "Capture is armed while it reads: device counters proved lossless\n"
        "input across a forced collection.\n\n"
        "Tier-1 library domain checks replace 62 silently wrong results with\n"
        "explicit errors.  The hot car/cdr opcodes remain permissive;\n"
        "(car 1) returns nil in this release.\n\n"
...
def fail(message):
    print(f"lisp65 2.0.0 offline verification: FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
if not (value.get("format") == "lisp65-v2.0.0-release-package-v1"
...
if not (isinstance(rows, list) and len(rows) == 19
        and product.get("artifact_count") == 19): fail("19-role inventory missing")
roles = set(); paths = set()
for row in rows:
...
if len(roles) != 19 or len(paths) != 19: fail("duplicate role or ship path")
...
if not (claims.get("native_init_l65") == "hardware-green-absent-valid-error"
        ...
        and claims.get("cursor_navigation") == "native-prompt-editor-hardware-green"
        and claims.get("capture_hybrid") == "armed-during-native-read-line"
        and claims.get("lossless_input") == "hardware-green-forced-collection"
        and claims.get("tier1_domain_discipline") == "qualified-62-cells"
        and claims.get("resident_delivery_chain") == "packed-byte-gated"
        and claims.get("car_cdr_tier2") == "not-delivered-documented-permissive"
        and claims.get("comfort_repl") == "not-delivered"
        and claims.get("matcher_blink") == "not-delivered"):
    fail("claim boundary drift")
print(f"lisp65 2.0.0 offline verification: PASS roles=19 files={len(files)} "
...
def build_product(root: Path, source_commit: str, clean: dict[str, Any],
    ...
    manifest = {
        ...
        "selected_variant": "v2.0-tier1-lossless-native-editor-stripped",
        ...
        "product": {
            ...
            "user_headroom": {"symbol_slots": 107, "namepool_bytes": 1467},
        ...
        "clean_build": {"builds": 2, "entry_point": clean["entry_point"],
                        ...
                        "source_commit": source_commit},
        "claims": {
            "banner": "WORKBENCH 2.0.0-on-physical-MEGA65",
            "native_init_l65": "hardware-green-absent-valid-error",
            "empty_journal_recovery": "hardware-green-practically-immediate",
            "cursor_navigation": "native-prompt-editor-hardware-green",
            "native_prompt_cursor_navigation": "hardware-green",
            "capture_hybrid": "armed-during-native-read-line",
            "lossless_input": "hardware-green-forced-collection",
            "tier1_domain_discipline": "qualified-62-cells",
            "resident_delivery_chain": "packed-byte-gated",
            "obsolete_v16core_library": "not-delivered",
            "car_cdr_tier2": "not-delivered-documented-permissive",
            ...
            "fast_typing_known_issue": "pensioned",
        },
...
def prepare(source_repository: Path, source_commit: str, product_root: Path,
    ...
    for source in (*DOCUMENTS.values(), *PROOFS.values()):
        relative = source.relative_to(ROOT).as_posix()
        candidate = subprocess.run(
            ["git", "show", f"{source_commit}:{relative}"],
            cwd=source_repository, stdout=subprocess.PIPE,
            stderr=subprocess.PIPE, check=False)
        require(candidate.returncode == 0
                and candidate.stdout == source.read_bytes(),
                f"release input differs from certified source: {relative}")
    ...
    manifest_b = build_product(stage_b, source_commit, clean, artifacts,
                               product_root.resolve())
    require(manifest_a == manifest_b, "v2.0 varied product staging differs")
    stage_verify = [COMMON.verify_product(stage_a), COMMON.verify_product(stage_b)]
    bundled_docs = [] if BUNDLE_DOCS_VALIDATOR is None else [
        BUNDLE_DOCS_VALIDATOR(stage_a), BUNDLE_DOCS_VALIDATOR(stage_b)]
    if bundled_docs:
        require(bundled_docs[0] == bundled_docs[1],
                "varied product staging has different bundled documents")
    ...
    require(product_a.read_bytes() == product_b.read_bytes(),
            "v2.0 product double-pack differs")
    product_archive_verify = [COMMON.verify_product_archive(product_a),
                              COMMON.verify_product_archive(product_b)]

    source_archive_entries = source_entries(
        source_repository, source_commit, clean)
    source_a = release_root / f"{TOP}-source-a.tar.gz"
    source_b = release_root / f"{TOP}-source-b.tar.gz"
    COMMON.deterministic_tar_gz(source_archive_entries, source_a)
    COMMON.deterministic_tar_gz(source_archive_entries, source_b)
    require(source_a.read_bytes() == source_b.read_bytes(),
            "v2.0 source double-pack differs")
    ...
    value = {
        ...
        "product": {
            ...
            "selected_variant":
                "v2.0-tier1-lossless-native-editor-stripped",
            "optional_library_D81": "not-delivered-editor-is-resident",
        ...
        "verification": {
            ...
            "bundled_documents": bundled_docs,
        },
        "assets": assets,
    ...
    PREPARATION_RECEIPT.parent.mkdir(parents=True, exist_ok=True)
    PREPARATION_RECEIPT.write_bytes(canonical(value))
    return value
...
def check(publish: Path) -> dict[str, Any]:
    ...
    require(publish.is_dir()
            ...
            "v2.0 publish directory is not the exact four-asset set")
...
def selftest() -> None:
    ...
    for key, value in (("path", "c"), ("bytes", 3),
        ...
        require(COMMON.package_set_sha(candidate) != baseline,
                f"v2.0 package mutation survived: {key}")
    with tempfile.TemporaryDirectory(prefix="lisp65-v200-tar-selftest-") as raw:
        ...
        require(first.read_bytes() == second.read_bytes(),
                "v2.0 deterministic archive selftest failed")
    print("c2-v200-release-package: SELFTEST PASS mutations=4 double-pack=1")
