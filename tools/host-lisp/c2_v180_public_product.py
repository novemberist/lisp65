#!/usr/bin/env python3
"""Rebuild the sealed v1.8.0 native-INIT/A0 product from public sources."""
...
PUBLIC = ROOT / "build/c2.3/v1.8.0-public-selected"
...
EXPECTED_RAW = {
    "PRG": (41566,
        "915de4d6c41199c7336ad1bb917bb66278918147461a1b9926be7c530682ae58"),
    "ELF": (648636,
        "74ac662a9aa87e4673ec4fd4c96dec18a58bd14089afa5f059656d557e50dfc4"),
    "profile": (13286,
        "9530d7f94012eee8fb680c71f981ccd527c4bb34b28df002bf7aa53a787e14ed"),
}
...
def materialize_predecessor() -> dict[str, Any]:
    ...
    if not predecessor_ready():
        ...
        V160.STATIC_RECEIPT.write_bytes(canonical({
            "status": "PASS: V1.8 PUBLIC PREDECESSOR STATIC PLANE",
...
def load_successor_modules() -> None:
    ...
    if INIT is None:
        ...
        CARD = importlib.import_module("c2_v180_release_card")
        RELEASE_MEDIA = importlib.import_module("c2_v180_release_media")
...
def configure_paths() -> None:
    ...
    INIT.DRIVER = DRIVER
    INIT.FORMAT = "lisp65-c2-v180-public-product-v1"
    INIT.STATUS = "PASS: V1.8.0 PUBLIC SOURCE PRODUCT"
    INIT.BASELINE_SPECS = predecessor_specs()
    INIT.BASELINE_STDLIB = INIT.BASELINE_SPECS[0][2]
    INIT._configure_plane_module()
    CARD.RELEASE.EXPECTED_BANNER = "WORKBENCH 1.8.0"
    CARD.RELEASE.PREDECESSOR_BANNER = "WORKBENCH 1.7.0"


def build_static() -> dict[str, Any]:
    require(not PUBLIC.exists(), "v1.8 public source stage is one-shot")
    ...
    require(plane["geometry"]["bytes"] == 46053
            ...
            "public v1.8 source plane geometry drift")
    value = {
        "format": "lisp65-v1.8-public-static-plane-v1",
        "status": "PASS: V1.8 PUBLIC NATIVE-INIT SOURCE PLANE",
    ...
    print("v1.8 public product: STATIC PASS images=6 evidence=0")
    return value
...
def configure_link() -> Any:
    ...
    CARD.configure()
    # The public producer consumes only current source/configuration.  The
    # private qualification receipt selected the byte target but is not a
    # build input in either clean clone.
    CARD.BASE.authority = lambda: {
        "authority": "public-current-source-v1.8",
        "private_evidence_inputs": 0,
        "release": "v1.8.0",
    }
    return CARD.BASE
...
def build_link() -> dict[str, Any]:
    require(STATIC_RECEIPT.is_file() and not BUILD.exists(),
            "v1.8 public link lifecycle drift")
    configure_link()
    ...
    require(all(path.is_file() for path in (PRG, ELF, PROFILE)),
            f"public v1.8 producer emitted no final pair (exit={exit_code})")
    ...
    for role, identity in observed.items():
        require((identity["bytes"], identity["sha256"]) == EXPECTED_RAW[role],
                f"public v1.8 {role} differs from sealed raw link")
    value = {
        "format": "lisp65-v1.8-public-link-v1",
        "status": "PASS: V1.8 PUBLIC CLOSED-CAPTURE SOURCE LINK",
        "private_evidence_inputs": 0, "WPLTO_runs": 1,
        "product_links": 1, "artifacts": observed,
        "configuration": {"native_INIT_L65": True,
            "recovery_quiescence_A0": True, "Capture_present": True,
            "Capture_initial_tail": 255, "losslessness_claim": False,
            "Comfort": False,
            "Block_3": False, "diagnostic": False},
    }
    LINK_RECEIPT.write_bytes(canonical(value))
    print("v1.8 public product: LINK PASS WPLTO=1 evidence=0")
    return value
...
def configure_completion() -> None:
    ...
    def current_semantics() -> None:
        ...
        require(section.bytes == 40,
                "public v1.8 verifier-binding geometry drift")
...
def public_acceptance_projection(elf: Path) -> dict[str, Any]:
    """Project the v1.8 active freight union over the fixed v5 Golden."""
    golden_module = V160.ACCEPT.V5_GOLDEN
    golden = load(golden_module.GOLDEN)
    expected_sha = getattr(golden_module, "GOLDEN_SHA256",
                           sha(golden_module.GOLDEN))
    ...
    try:
        layout = V160.ACCEPT.LAYOUT.layout_from_elf(elf)
        registries, registered = V160.ACCEPT._active_freight_union()
        rows = V160.ACCEPT._freight_proof_rows(layout, registries)
        additive = V160.ACCEPT._additive_section_closure(
    ...
    additive["placement_gate"] = {"gate": "active-card-registry-union",
        "status": "passed", "registries": [row["registry"] for row in registries],
        "proof_rows": rows}
    expected = [".lisp65_c2_kernal_window.input_capture_helper",
    ...
    require(comparison.get("comparison") ==
            ...
            and additive["registered_sections"] == expected,
            "public v1.8 additive acceptance drift")
...
def complete_product() -> dict[str, Any]:
    ...
    V160.public_acceptance_projection = public_acceptance_projection
    V160.configure_completion_paths()
    product = V160.CAN.WPLTO / "lisp65-c2-substitution-linked.prg"
    ...
    V160.SOURCE_MEDIA.FLOW.BASE.INV = accepted
    V160.CRC_MEDIA.INV = accepted
    V160.SOURCE_MEDIA.card_projection = lambda: {
        "acceptance": {"VMA_golden": acceptance["VMA_golden"]}}
    original_configure = V160.CAN.REPLAY.configure
    original_fixed = V160.PRODUCT.FIXED_BLOCK_LEAF.audit_elf
    original_facade = V160.PRODUCT.fixed_facade_gate
    original_kernal = V160.PRODUCT.kernal_freedom_gate

    def fixed(candidate: Path, **kwargs: Any) -> dict[str, Any]:
        return V160.SOURCE_MEDIA._link105_fixed_audit(
    ...
    def facade(out: Path, target: Path, suffix: str) -> dict[str, Any]:
        ...
        if not report.exists():
            V160.NESTED_MEDIA.materialize_candidate_publish_predecessors(
                out, target, Path(str(target) + ".elf"))
        value = V160.CRC_MEDIA._current_facade_gate(
            original_facade, out, target, suffix)
        value["packed_PRG_facade"] = V160.FACADE.packed_facade_gate(
            target, Path(str(target) + ".elf"))
        return value

    def v18_kernal(out: Path, target: Path) -> dict[str, object]:
        expected = {*map(str,
            V160.PRODUCT.INPUT_CAPTURE_BUILD_CONFIGURATION["allocated"]),
            *map(str,
            V160.PRODUCT.INPUT_HYBRID_BUILD_CONFIGURATION["allocated"])}
        truth = V160.ElfTruth.read(Path(str(target) + ".elf"),
            llvm_readobj=V160.PRODUCT.TOOLCHAIN / "llvm-readobj")
        observed = {row.name for row in truth.sections}
        require(expected <= observed,
                "public v1.8 ELF lost registered Capture KERNAL freight")
        return original_kernal(out, target)

    V160.CAN.REPLAY.configure = lambda: None
    V160.PRODUCT.FIXED_BLOCK_LEAF.audit_elf = fixed
    V160.PRODUCT.fixed_facade_gate = facade
    V160.PRODUCT.kernal_freedom_gate = v18_kernal
    try:
        value = V160.CAN.complete_artifacts()
    finally:
        V160.CAN.REPLAY.configure = original_configure
        V160.PRODUCT.FIXED_BLOCK_LEAF.audit_elf = original_fixed
        V160.PRODUCT.fixed_facade_gate = original_facade
        V160.PRODUCT.kernal_freedom_gate = original_kernal
    final_product = V160.CAN.FINAL / product.name
    ...
    V160.FACADE.packed_facade_gate(final_product, final_elf)
    receipt = PUBLIC / "completion.json"
    receipt.write_bytes(canonical({
        "status": "PASS: V1.8 PUBLIC ARTIFACT-ONLY COMPLETION",
        "private_evidence_inputs": 0, "compiler_runs": 0, "linker_runs": 0,
        "acceptance": acceptance,
        "completion": bind(V160.CAN.RECEIPTS / "artifact-completion.json")}))
    print("v1.8 public product: COMPLETION PASS rebuilds=0")
    return value
...
def canonical_manifest() -> dict[str, Any]:
    ...
    static = {"status": "passed-v1.8.0-release-static-plane",
              "product_build_id": product["product_build_id_hex"],
              "bank2_static_code_bytes": 46053}
    wplto = {"status": "passed-one-public-v1.8-source-link",
             "product": bind(PRG)}
    ...
    require(len(prefix) == 46053, "public v1.8 Bank-2 prefix drift")
    sections = RELEASE_MEDIA.BASE.MEDIA.mapped_section_rows(truth)
    base = 0x20000
    ...
    require(cursor == 0x2BFD1 and end - base == 49105,
            "public v1.8 composed Bank-2 geometry drift")
    ...
    value["static_plane"].update({
        ...
        "membership_authority": "v1.8 final-ELF composed ownership",
    })
...
def build_shared_media() -> dict[str, Any]:
    ...
    require(len(rows) == 19, "public v1.8 shared-media role count drift")
    for row in rows:
        expected_role(row["role"], ROOT / row["path"])
    print("v1.8 public product: SHARED MEDIA PASS roles=19")
    return value
...
def build_library_media() -> dict[str, Any]:
    ...
    suite_path = generated / "v16core-v180-suite.json"
    ...
    suite_path.write_bytes(canonical({
        ...
        "description": "sealed v1.6 cursor editor for v1.8 public build",
    }))
    ...
    require(located == artifact, "public v1.8 v16core locator drift")
    ...
    require(len(decoded) == 1 and decoded[0]["name"] == "v16core",
            "public v1.8 library row closure drift")
    ...
    require(visible == {b"L65INDEX": index, b"V16CORE": artifact},
            "public v1.8 library visible closure drift")
    ...
    print("v1.8 public product: LIBRARY MEDIA PASS rows=1")
    return value
...
def build_selected() -> dict[str, Any]:
    ...
    require(shared["artifact_count"] == len(shared["artifacts"]) == 19,
            "public v1.8 shared-media closure absent")
    ...
    require(len(rows) == len({row["role"] for row in rows}) == 22,
            "public v1.8 selected role inventory drift")
    authority = load(AUTHORITY)
    value = {
        "format": "lisp65-v1.8-public-selected-product-v1",
        "status": "passed-public-source-selected-v1.8-closed-capture-product",
        ...
        "private_evidence_inputs": 0,
        "selector": "v1.8-closed-capture-substrate", "artifacts": rows}
    require(value["artifact_set_sha256"] ==
                authority["sealed_product_artifact_set_sha256"],
            "public v1.8 artifact set differs from candidate seal")
    MANIFEST_OUT.write_bytes(canonical(value))
    print("v1.8 public product: SELECTED PASS roles=22 " +
...
def check() -> dict[str, Any]:
    ...
    rows = value.get("artifacts", [])
    authority = load(AUTHORITY)
    require(value.get("format") == "lisp65-v1.8-public-selected-product-v1"
            and value.get("private_evidence_inputs") == 0
            and value.get("selector") == "v1.8-closed-capture-substrate"
            and value.get("artifact_count") == len(rows) == 22
            and artifact_set(rows) == value.get("artifact_set_sha256")
                == authority["sealed_product_artifact_set_sha256"],
            "selected public v1.8 manifest drift")
    for row in rows:
        expected_role(row["role"], ROOT / row["path"])
    print("v1.8 public product: CHECK PASS roles=22 evidence=0")
    return value
...
def run_stage(action: str) -> None:
    ...
    require(result.returncode == 0,
            f"v1.8 public {action} stage red:\n{result.stdout}")
    print(result.stdout.strip())


def build() -> None:
    require(not PUBLIC.exists(), "v1.8 public build is one-shot")
    for action in ("static", "link", "complete", "media", "selected"):
        run_stage(action)
    check()
    print("v1.8 public product: FULL PASS roles=22 evidence=0")
...
if __name__ == "__main__":
    try:
        ...
        print(f"v1.8 public product: FIRST RED: {error}", file=sys.stderr)
        raise SystemExit(2)
