#!/usr/bin/env python3
"""Prepare and twice verify the four immutable v1.9.0 Halt-B assets."""
...
DEFAULT_CLEAN_RECEIPT = ROOT / (
    "build/release-v1.9.0/v1.9.0-public-clean-build-receipt.json")
...
DOCUMENTS = {
    "docs/release-notes.md": ROOT / "docs/releases/1.9.0.md",
...
def artifact_sources(product_root: Path) -> dict[str, dict[str, Any]]:
    authority = load(AUTHORITY, "v1.9 public authority")
    ...
    require(
        ...
        "selected v1.9 product manifest drift")
    for role, row in rows.items():
        ...
        require(source.is_file() and not source.is_symlink()
                ...
                f"selected v1.9 product byte drift: {role}")
        row["source"] = source
    return rows
...
def validate_authorities(product_root: Path, clean_receipt: Path,
    ...
    authority = load(AUTHORITY, "v1.9 public authority")
    clean = load(clean_receipt, "v1.9 clean-build receipt")
    sources = artifact_sources(product_root)
    require(
        authority.get("format") == "lisp65-c2-public-build-authority-v7"
        ...
        and clean.get("format")
            == "lisp65-c2-v190-public-clean-build-receipt-v1"
        ...
        and clean.get("artifact_set_sha256")
            == authority["sealed_product_artifact_set_sha256"],
        "v1.9 public authority/clean-build mismatch")
...
def readme(product_set: str) -> bytes:
    return (
        "LISP65 WORKBENCH 1.9.0\n"
        "========================\n\n"
        "This is the owner-Ship-selected v1.9 release package.\n"
...
def fail(message):
    print(f"lisp65 1.9.0 offline verification: FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
if not (value.get("format") == "lisp65-v1.9.0-release-package-v1"
...
print(f"lisp65 1.9.0 offline verification: PASS roles=19 files={len(files)} "
...
def build_product(root: Path, source_commit: str, clean: dict[str, Any],
    ...
    manifest = {
        "format": "lisp65-v1.9.0-release-package-v1",
        ...
        "product": {
            ...
            "user_headroom": {"symbol_slots": 109, "namepool_bytes": 1486},
        ...
        "claims": {
            "banner": "WORKBENCH 1.9.0-on-physical-MEGA65",
...
def prepare(source_repository: Path, source_commit: str, product_root: Path,
    ...
    release_root = ROOT / "build/release-v1.9.0"
    ...
    manifest_b = build_product(stage_b, source_commit, clean, artifacts,
                               product_root.resolve())
    require(manifest_a == manifest_b, "v1.9 varied product staging differs")
    ...
    require(product_a.read_bytes() == product_b.read_bytes(),
            "v1.9 product double-pack differs")
    ...
    require(source_a.read_bytes() == source_b.read_bytes(),
            "v1.9 source double-pack differs")
    ...
    value = {
        "format": "lisp65-v190-release-package-preparation-v1",
    ...
    receipt = release_root / "v1.9.0-package-preparation-receipt.json"
...
def check(publish: Path) -> dict[str, Any]:
    ...
    require(publish.is_dir()
            ...
            "v1.9 publish directory is not the exact four-asset set")
    ...
    require(manifest.get("source_commit") == clean.get("source_commit")
            ...
            "v1.9 release asset authority mismatch")
...
def selftest() -> None:
    ...
    for key, value in (("path", "c"), ("bytes", 3),
        ...
        require(COMMON.package_set_sha(candidate) != baseline,
                f"v1.9 package mutation survived: {key}")
    with tempfile.TemporaryDirectory(prefix="lisp65-v190-tar-selftest-") as raw:
        ...
        require(first.read_bytes() == second.read_bytes(),
                "v1.9 deterministic archive selftest failed")
    print("c2-v190-release-package: SELFTEST PASS mutations=4 double-pack=1")
...
def main() -> int:
    ...
    prepare_parser.add_argument("--output", type=Path,
                                default=ROOT / "build/release-v1.9.0/publish")
