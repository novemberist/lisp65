#!/usr/bin/env python3
"""Package/check the isolated v1.2.2 candidate with canonical media code."""
...
import c2_v122_candidate_product as PRODUCT  # noqa: E402


BUILD = ROOT / "build/c2.2/v1.2.2-candidate-media"
MANIFEST = BUILD / "candidate-manifest.json"


def configure() -> None:
    PRODUCT.PRODUCT.configure()
    CAN.MANIFEST = PRODUCT.PRODUCT.MANIFEST
    MEDIA.CANONICAL = CAN
    MEDIA.BUILD = BUILD
    MEDIA.PRODUCT_MANIFEST = PRODUCT.PRODUCT.MANIFEST
...
def main() -> int:
    ...
    if args.action == "build":
        MEDIA.check()
    print(
        "c2-v1.2.2-candidate-media: PASS "
...
if __name__ == "__main__":
    try:
        ...
        MEDIA.MediaError, CAN.CanonicalError, RuntimeError, OSError,
        ValueError, KeyError, json.JSONDecodeError,
    ) as error:
        print(
            "c2-v1.2.2-candidate-media: FIRST RED: " + str(error),
            file=sys.stderr,
        )
        raise SystemExit(2)
