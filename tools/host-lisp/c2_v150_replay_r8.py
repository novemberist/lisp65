#!/usr/bin/env python3
"""Run the authorized Link-97 r8 artifact replay without source rebinding."""
...
BUILD = CARD.BUILD
R7 = BUILD / "post-link-qualification-replay-r7"
R8 = BUILD / "post-link-qualification-replay-r8"
R7_FIRST_RED = CARD.ROOT / (
    "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
    "c2.3-v1.5.0-link97-post-link-qualification-replay-r7-first-red.json")
AUTHORIZATION = "37f12ed3"


def configure_replay() -> None:
    CARD.REPLAY_PREVIOUS_RED = R7
    CARD.REPLAY = R8
    CARD.REPLAY_PROFILE = R8 / "candidate-profile.json"
    CARD.REPLAY_INTERNAL = R8 / "wplto-internal.json"
    CARD.REPLAY_LINKED_GATE = R8 / "single-submit-linked-gates.json"
    CARD.REPLAY_FIRST_RED_RECEIPT = R7_FIRST_RED
...
def complete_in_fresh_process() -> None:
    ...
    CARD.require(result.returncode == 0,
                 "v1.5 r8 fresh-process completion red:\n" + result.stdout)
...
def selftest() -> int:
    configure_replay()
    CARD.require(
        R7.is_dir() and (R7 / "candidate-profile.json").is_file()
        and R7_FIRST_RED.is_file()
        and not R8.exists()
        and CARD.SLICE_MAP.RECEIPT
            == CARD.SLICE_MAP.REBOUND_RECEIPT
        and CARD.SLICE_MAP.REBOUND_RECEIPT.is_file()
        and CARD.REPLAY_AUTHORIZATIONS[-1] == AUTHORIZATION,
        "v1.5 r8 replay adapter boundary red")
    CARD.selftest()
    print("v1.5 Link-97 r8 replay adapter selftest: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"v1.5 Link-97 r8 replay: FIRST RED: {error}")
        raise SystemExit(2)
