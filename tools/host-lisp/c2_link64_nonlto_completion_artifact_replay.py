#!/usr/bin/env python3
"""Qualify the sole Link-64 non-LTO mode-length WPLTO artifact without linking."""
...
OUT = ROOT / f"build/c2.2/substitution/{STEM}-artifact-replay"
PRODUCT = OUT / SOURCE_PRODUCT.name
ELF = Path(str(PRODUCT) + ".elf")
MAP = Path(str(PRODUCT) + ".map")
RECEIPT = EVIDENCE / f"c2.2-{STEM}-artifact-replay-receipt.json"
EXPECTED_REGION1 = {
...
def configure() -> None:
    ...
    require(
        ...
        "current Link-64 profile geometry drift")
    require(
        ...
        "current Link-64 Boot inventory drift")
...
def main() -> int:
    ...
    require(
        ...
        and source_gate["format"]
            == "lisp65-c2-cpu-chip-write-completion-probe-v6"
        and source_gate["mutation_count"] == 24,
        "Link-64 expected Class-A checker boundary drift")
    before = snapshot(SOURCE)
    require(
        before and all((int(row["mode"], 8) & 0o222) == 0
                       for row in before.values()),
        "Link-64 WPLTO tree is not immutable")
    ...
    try:
        ...
        retry_length = LENGTH.audit_elf(ELF)
        ...
        P.write(OUT / "c2-f011-mount-window-gate.json",
                json.dumps(f011, indent=2, sort_keys=True) + "\n")
        handoff = P.handoff_z_abi_gate(OUT, PRODUCT, "link64-replay")
        pre_ownership = P.pre_ownership_gate(
            OUT, PRODUCT, "link64-replay")
        data_reference = P.profile_data_reference_gate(
            OUT, PRODUCT, "link64-replay", pre_ownership)
        facade = P.fixed_facade_gate(OUT, PRODUCT, "link64-replay")
        fixed = P.FIXED_BLOCK_LEAF.audit_elf(
            ELF, out=OUT / "fixed-block-rtov-fail-link64.json")
        ...
        P.write(OUT / "runtime-overlays-final.bin",
    ...
    require(
        ...
        and retry_length["mutation_count"] == 9
        ...
        "Link-64 artifact replay has a structural or capacity red")
    require(before == snapshot(SOURCE),
            "artifact replay modified the immutable Link-64 WPLTO tree")

    value = {
        "format": "lisp65-c2-link64-nonlto-mode-length-artifact-replay-v1",
        ...
        "authority": {
            ...
            "retry_length_ELF_gate": bind(Path(LENGTH.__file__)),
            "assembler_ABI_gate": bind(Path(ABI.__file__)),
            "driver": bind(Path(__file__)),
        },
        "class_A_gate_correction": {
            "old_model":
                "required one immediate LDA form and missed the LDA/STA handoff",
            "current_model":
                "proves initial handoff, canonical mode byte, three "
                "rematerializations and retry-edge dominance",
        ...
        "next_gate": "one authorized Link 64 with all gates fresh",
        "claim_limit": (
            "Artifact-side WPLTO qualification only. No product Link 64, "
    ...
    print(
        "c2-link64-nonlto-completion-artifact-replay: PASS "
        f"text={walls['bank0_text_headroom_bytes']} "
        f"e000={walls['e000_headroom_bytes']} "
        f"session={session['storage']['size']} "
        f"rematerializations="
        f"{retry_length['linked_dataflow']['rematerialization_call_count']}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-link64-nonlto-completion-artifact-replay: FIRST RED: "
            + str(error),
            file=sys.stderr)
        raise SystemExit(2)
