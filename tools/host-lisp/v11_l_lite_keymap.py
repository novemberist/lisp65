...
def render_docs(value: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"This generated table is bundled with lisp65 {json.loads((ROOT / 'config/c2-v220-bundle-docs.json').read_text())['release']}.",
        "",
...
def selftest(value: dict[str, Any]) -> None:
    validate(value)
    approved = json.loads((ROOT / 'config/c2-v220-bundle-docs.json').read_text())
    rendered = render_docs(value)
    require(rendered == DOC_OUT.read_text(), "approved keymap document differs")
    for foreign in ("obsolete-release", "unapproved-successor"):
        changed = rendered.replace(approved["release"], foreign)
        require(changed != DOC_OUT.read_text(), "keymap version drift survived")
    ...
    print("v11-l-lite-keymap: SELFTEST PASS mutations=6 exact-mx=true "
