#!/usr/bin/env python3
"""Bind the Link-71 hardware oracle to canonical SESS-record media."""
...
CONFIG = ROOT / (
    "config/c2.2-defstruct-link71-session-record-identity-replay.json")
LINK = EVIDENCE / (
...
OUT = ROOT / os.environ.get(
    "C2_DEFSTRUCT_SESS_OUT",
    "build/post-promotion/link71-defstruct-session-record-identity-"
    "hardware-replay")
RECEIPT = ROOT / os.environ.get(
    "C2_DEFSTRUCT_SESS_RECEIPT",
    "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
    "c2.2-link71-defstruct-session-record-identity-hardware-receipt.json")
HARNESS_FIRST_RED = EVIDENCE / (
    "c2.2-link71-defstruct-session-record-identity-hardware-harness-"
    "first-red.json")
REMOTE_MEDIA = os.environ.get("C2_DEFSTRUCT_SESS_REMOTE", "L71SESS.D81")
...
def prepare() -> None:
    ...
    deployment.update({
        "format":
            "lisp65-c2.2-link71-defstruct-session-record-identity-"
            "deployment-v1",
        "remote_media": REMOTE_MEDIA,
        "media_transport":
            "upload/readback/mount before product load; later loads preserve "
            "the mounted canonical-SESS medium",
    })
...
def finalize() -> None:
    ...
    receipt.update({
        "format":
            ...
            "hardware-v1",
        "status":
            "passed-Link71-canonical-SESS-record-require-defstruct-"
            "on-hardware",
    })
    ...
    receipt["claim_limit"] = (
        "Link 71 require/defstruct freight and canonical SESS-record media "
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-defstruct-link71-session-record-identity-hw: FIRST RED: "
            + str(error))
        raise SystemExit(1)
