#!/usr/bin/env python3
"""Give the already verified Link-62 C1 carrier its final identity.

This is an artifact-only rename and receipt normalization.  It runs no
compiler, linker, product mutation, or hardware action.
"""
...
import c2_c1_freezer_carrier_link62 as LINK62  # noqa: E402


OUT = LINK62.OUT
RECEIPT = LINK62.RECEIPT
OLD_MAIN = OUT / (
...
MAIN = OUT / "runtime-overlays-session-c1-freezer-link62-stage-bound.bin"
REGION1 = OUT / "runtime-overlays-session-c1-freezer-link62-region1.bin"
MANIFEST = OUT / (
...
def main() -> int:
    ...
    manifest_source = OLD_MANIFEST if OLD_MANIFEST.is_file() else MANIFEST
    ...
    require(
        ...
        and binding["match"]
        and binding["session_crc16"] == "0x8d75"
        and binding["tail_width_bytes"] == 6,
        "unfinished Link-62 carrier authority drift")
    ...
    manifest["format"] = (
        "lisp65-C1-Freezer-Link62-v4-rebound-stage-bound-family-v1")
    manifest["status"] = (
        "passed-nonpromotable-Link62-carrier-awaiting-hardware")
    manifest["outer_link62_stage_binding"] = manifest.pop(
        "outer_link60_stage_binding")
    relocation = manifest["relocation_rebind"]
    relocation["external_sites_already_link62_exact"] = relocation.pop(
        "external_sites_already_link60_exact")
    for row in relocation["sites"]:
        row["link62_encoded"] = row.pop("link60_encoded")
        row["link62_value"] = row.pop("link60_value")
    for row in manifest["slice_provenance"]:
        row["source"] = row["source"].replace("Link60", "Link62")
    ...
    authority["immutable_link62_product"] = authority.pop(
        "immutable_link60_product")
    authority["link62_elf"] = authority.pop("link60_elf")
    authority["link62_receipt"] = authority.pop("link60_receipt")
    authority["link62_rebind_driver"] = BASE.bind(
        ROOT / "tools/host-lisp/c2_c1_freezer_carrier_link62.py")
    authority["artifact_identity_completion"] = BASE.bind(Path(__file__))
    receipt["capacity"]["deployed_resident_authority"] = "immutable Link 62"
    construction = receipt["construction"]
    construction["region1_byteidentical_Link62"] = construction.pop(
        "region1_byteidentical_Link60")
    tail = manifest["outer_link62_stage_binding"]
    ...
    receipt["proof"]["verifier_header"] = "byteidentical-Link62"
    ...
    os.chmod(OUT, 0o555)
    print(
        "c2-c1-freezer-carrier-link62-artifact-completion: PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-c1-freezer-carrier-link62-artifact-completion: FIRST RED: "
            + str(error),
            file=sys.stderr,
        )
        raise SystemExit(2)
