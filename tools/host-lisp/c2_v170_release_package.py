#!/usr/bin/env python3
"""Prepare and twice verify the four immutable v1.7.0 Halt-B assets."""
...
PREPARED_ON = "2026-08-27"
SOURCE_EPOCH = 1787788800
AUTHORITY = ROOT / "config/c2-v170-public-build-authority.json"
DEFAULT_CLEAN_RECEIPT = ROOT / (
    "build/release-v1.7.0/v1.7.0-public-clean-build-receipt.json")
...
DOCUMENTS = {
    "docs/release-notes.md": ROOT / "docs/releases/1.7.0.md",
...
PROOFS = {
    "proof/release-device-session.json": ROOT / (
        ...
        "c2.3-v1.7.0-release-d-session-result-receipt.json"),
    "proof/init-device-session.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v1.7-init-l65-product-variants-device-result-receipt.json"),
    "proof/d5-delta-attribution.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v1.7.0-release-d5-delta-attribution-receipt.json"),
    "proof/candidate-seal.json": ROOT / (
        "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
        "c2.3-v1.7.0-candidate-seal-receipt.json"),
}
...
def artifact_sources(product_root: Path) -> dict[str, dict[str, Any]]:
    authority = load(AUTHORITY, "v1.7 public authority")
    ...
    require(
        ...
        "selected v1.7 product manifest drift")
    for role, row in rows.items():
        ...
        require(source.is_file() and not source.is_symlink()
                ...
                f"selected v1.7 product byte drift: {role}")
        row["source"] = source
    return rows
...
def validate_authorities(product_root: Path, clean_receipt: Path,
    ...
    authority = load(AUTHORITY, "v1.7 public authority")
    clean = load(clean_receipt, "v1.7 clean-build receipt")
    sources = artifact_sources(product_root)
    require(
        authority.get("format") == "lisp65-c2-public-build-authority-v5"
        and authority.get("release") == RELEASE
        and authority.get("selected_variant") == "v1.7-native-init-a0"
        and authority.get("artifact_count") == 22
        and clean.get("format")
            == "lisp65-c2-v170-public-clean-build-receipt-v1"
        and clean.get("status") == "passed"
        and clean.get("source_commit") == source_commit
        and clean.get("artifact_count") == 22
        and clean.get("selected_variant") == "v1.7-native-init-a0"
        ...
        and clean.get("artifact_set_sha256")
            == authority["sealed_product_artifact_set_sha256"],
        "v1.7 public authority/clean-build mismatch")
...
def readme(product_set: str) -> bytes:
    return (
        "LISP65 WORKBENCH 1.7.0\n"
        "========================\n\n"
        "This is the owner-Ship-selected v1.7 release package.\n"
        ...
        "  media/lisp65-library.d81  optional v1.7 libraries\n\n"
        "Optional package:\n"
...
def fail(message):
    print(f"lisp65 1.7.0 offline verification: FAIL: {message}", file=sys.stderr)
    raise SystemExit(1)
...
if not (value.get("format") == "lisp65-v1.7.0-release-package-v1"
...
if not (claims.get("native_init_l65") == "hardware-green-absent-valid-error"
        ...
        and claims.get("cursor_navigation") == "retained-v1.6-v16core"
        and claims.get("comfort_repl") == "not-delivered"
        and claims.get("matcher_blink") == "not-delivered"):
    fail("claim boundary drift")
print(f"lisp65 1.7.0 offline verification: PASS roles=22 files={len(files)} "
...
def build_product(root: Path, source_commit: str, clean: dict[str, Any],
    ...
    manifest = {
        "format": "lisp65-v1.7.0-release-package-v1",
...
def prepare(source_repository: Path, source_commit: str, product_root: Path,
    ...
    release_root = ROOT / "build/release-v1.7.0"
    ...
    manifest_b = build_product(stage_b, source_commit, clean, artifacts,
                               product_root.resolve())
    require(manifest_a == manifest_b, "v1.7 varied product staging differs")
    ...
    require(product_a.read_bytes() == product_b.read_bytes(),
            "v1.7 product double-pack differs")
    ...
    require(source_a.read_bytes() == source_b.read_bytes(),
            "v1.7 source double-pack differs")
    ...
    value = {
        "format": "lisp65-v170-release-package-preparation-v1",
    ...
    receipt = release_root / "v1.7.0-package-preparation-receipt.json"
...
def check(publish: Path) -> dict[str, Any]:
    ...
    require(publish.is_dir()
            ...
            "v1.7 publish directory is not the exact four-asset set")
    ...
    require(manifest.get("source_commit") == clean.get("source_commit")
            ...
            "v1.7 release asset authority mismatch")
...
def selftest() -> None:
    ...
    for key, value in (("path", "c"), ("bytes", 3),
        ...
        require(COMMON.package_set_sha(candidate) != baseline,
                f"v1.7 package mutation survived: {key}")
    with tempfile.TemporaryDirectory(prefix="lisp65-v170-tar-selftest-") as raw:
        ...
        require(first.read_bytes() == second.read_bytes(),
                "v1.7 deterministic archive selftest failed")
    print("c2-v170-release-package: SELFTEST PASS mutations=4 double-pack=1")
...
def main() -> int:
    ...
    prepare_parser.add_argument("--output", type=Path,
                                default=ROOT / "build/release-v1.7.0/publish")
