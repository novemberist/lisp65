#!/usr/bin/env python3
"""Prepare/check the fresh v1.5 name-freight D1 boot."""
...
CONFIG = ROOT / "config/c2-v150-name-freight-d1.json"
RUNNER = ROOT / "scripts/c2-v150-name-freight-d1-hw.sh"
MEDIA = ROOT / (
...
OUT = ROOT / "build/c2.3/v1.5.0-name-freight-d1"
FORMAT = "lisp65-c2.3-v1.5.0-name-freight-d1-preparation-v1"
STATUS = "V150-NAME-FREIGHT-D1-PREPARED-NOT-RUN"
...
def contract(value: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    media = load(MEDIA)
    require(value.get("format") == "lisp65-c2-v150-name-freight-d1-v1"
            ...
            "name-freight D1 contract/media authority drift")
    return value
...
def runner_gate(source_override: str | None = None) -> dict[str, Any]:
    ...
    require("dry-run|stage|confirm-liveness" in source
            ...
            "name-freight D1 runner lifecycle token absent")
    ...
    require("mega65_ftp" not in live and "$FTP" not in live
            ...
            "name-freight D1 liveness window admits device access")
    require(all(f"grep -Fqx '{sign}'" in source for sign in SIGNS),
            "name-freight D1 can omit owner liveness confirmation")
...
def mutations(config: dict[str, Any], source: str) -> list[str]:
    ...
    for name, key, value in (
        ("short-window", "access_free_seconds", 20),
        ("wrong-media", "successor_media_authority", "old"),
    ):
        def run(key=key, value=value) -> None:
            trial = deepcopy(config); trial[key] = value; contract(trial)
        cases[name] = run
    changed = deepcopy(config); changed["required_visible_liveness"].pop(0)
    cases["drop-sign"] = lambda: contract(changed)
    source_cases = {
    ...
    require(rejected == list(cases), "name-freight D1 mutation survived")
    return rejected
...
def derive_preparation() -> dict[str, Any]:
    ...
    return {
        ...
        "claim_limit": (
            "Fresh name-freight D1 preparation only. D2-D5 remain closed until "
...
def validate_preparation(value: dict[str, Any], *, verify: bool) -> None:
    require(value.get("format") == FORMAT and value.get("status") == STATUS
            ...
            "name-freight D1 preparation claim drift")
    if verify:
        require(value == derive_preparation(), "name-freight D1 preparation stale")
...
def verify_terminal(text: Path, image: Path) -> dict[str, Any]:
    ...
    require("WORKBENCH 1.5.0" in raw and "lisp65>" in raw,
            "name-freight D1 banner/prompt absent")
...
def derive_result() -> dict[str, Any]:
    ...
    signs = (OUT / "owner-visible-signs.txt").read_text(encoding="utf-8").splitlines()
    require(signs == SIGNS and (OUT / "owner-visible-signs-confirmed").is_file(),
            "name-freight D1 owner liveness confirmation absent")
    return {
        "format": "lisp65-c2.3-v1.5.0-name-freight-d1-v1",
        "recorded_on": "2026-08-11",
        "status": "V150-NAME-FREIGHT-D1-GREEN",
        ...
        "execution_accounting": {"hardware_contacts": 1, "forms": 0,
                                 "product_links": 0, "WPLTO_runs": 0},
        "unlock": {"D2_D5": True},
        "claim_limit": "Fresh D1 only; D2-D5 have not run.",
    }
...
def main() -> int:
    ...
    if args.action == "prepare":
        require(not PREP.exists(), "name-freight D1 preparation exists")
        value = derive_preparation(); PREP.write_bytes(canonical(value))
        print("v1.5 name-freight D1 preparation: PASS mutations=7")
    elif args.action == "check":
        validate_preparation(load(PREP), verify=True)
        print("v1.5 name-freight D1 check: PASS mutations=7")
    elif args.action == "selftest":
        validate_preparation(derive_preparation(), verify=False)
        print("v1.5 name-freight D1 selftest: PASS mutations=7")
        ...
        print("v1.5 name-freight D1 terminal: PASS")
    else:
        require(not RESULT.exists(), "name-freight D1 result exists")
        RESULT.write_bytes(canonical(derive_result()))
        print("v1.5 name-freight D1: PASS D2-D5 now permitted")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"V1.5 NAME FREIGHT D1: {message}", file=sys.stderr)
        raise SystemExit(1)
