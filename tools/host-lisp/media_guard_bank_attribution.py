#!/usr/bin/env python3
"""Generate or verify the mid-transaction media-guard Bank-0 attribution."""
...
BASELINE_ARCHIVE = ROOT / (
    ...
    "r4-product-candidate-07de3cf.tar.gz"
)
BASELINE_ELF_MEMBER = (
...
CANDIDATE_ARCHIVE = ROOT / (
    "tests/bytecode/dialect-v2/evidence/promotions/"
    "r4-product-candidate-18d8c56.tar.gz"
)
CANDIDATE_RECEIPT_MEMBER = (
    "payload/tests/bytecode/dialect-v2/evidence/r3/"
    "canonical-product-reproducibility-receipt.json"
)
CANDIDATE_ELF_MEMBER = (
    "payload/build/products/workbench/overlay-stack-guard/"
    "lisp65-workbench-overlay-linked.prg.elf"
)
REPORT = ROOT / (
    "tests/bytecode/dialect-v2/evidence/r3/"
    "media-guard-bank-attribution-receipt.json"
)
NM = ROOT / "tools/llvm-mos/bin/llvm-nm"
FORMAT = "lisp65-media-guard-bank-attribution-v1"
...
def symbol_row(
    name: str, baseline: tuple[int, int, str] | None,
    candidate: tuple[int, int, str] | None,
) -> dict[str, Any]:
...
def generate() -> dict[str, Any]:
    if sha_bytes(CANDIDATE_ARCHIVE.read_bytes()) != (
        "af85e2d7cd932176bdfc2786782babaf42cf9b3636f21a026412044089168426"
    ):
        raise AttributionError("sealed candidate archive binding drift")
    with tarfile.open(CANDIDATE_ARCHIVE, "r:gz") as archive:
        receipt_bytes = archive_member(archive, CANDIDATE_RECEIPT_MEMBER)
        candidate_bytes = archive_member(archive, CANDIDATE_ELF_MEMBER)
    try:
        receipt = json.loads(receipt_bytes)
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise AttributionError("sealed canonical product receipt is invalid") from exc
    if not isinstance(receipt, dict):
        raise AttributionError("sealed canonical product receipt must contain an object")
    ...
    if sha_bytes(candidate_bytes) != candidate_row.get("sha256"):
        raise AttributionError("sealed candidate ELF does not match the canonical double build")
    ...
    with tempfile.TemporaryDirectory(prefix="l65-media-guard-bank-") as directory:
    ...
    expected = {
        "vm_callprim": (6938, 6721),
        "io_disk_transaction_capture_mount_token": (0, 5),
        "lisp65_f011_scratch_buffer": (0, 56),
        "lisp65_f011_mount_token_op": (0, 62),
        "f011_issue_write_guarded": (0, 115),
        "io_disk_write_sector_guarded": (0, 142),
    }
    changes = []
    ...
    guard_bytes = sum(
        row["delta_bytes"] for row in changes if row["name"] != "vm_callprim"
    )
    consolidation_credit = -next(
        row["delta_bytes"] for row in changes if row["name"] == "vm_callprim"
    )
    ...
    if (
        baseline_bank != 553 or candidate_bank != 381
        or guard_bytes != 380 or consolidation_credit != 217
        or named_delta != 163 or alignment_delta != 9 or resident_delta != 172
        or baseline_bank - candidate_bank != 172
    ):
    ...
    return {
        "format": FORMAT,
        "version": 1,
        "id": "mid-transaction-media-guard-bank-debit",
        "status": "pass",
        ...
        "candidate": {
            "canonical_receipt": {
                ...
                "sha256": sha_bytes(receipt_bytes),
            },
        ...
        "attribution": {
            ...
            "guard_path_bytes": guard_bytes,
            "vm_callprim_consolidation_credit_bytes": consolidation_credit,
            "named_symbol_delta_bytes": named_delta,
            "alignment_delta_bytes": alignment_delta,
            "resident_delta_bytes": resident_delta,
            "bank_debit_bytes": baseline_bank - candidate_bank,
            "reconciliation": (
                "guard-paths-minus-vm-callprim-credit-plus-alignment-"
                "equals-resident-delta-and-bank-debit"
            ),
        },
        "result": "bank-debit-fully-attributed",
    }
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "generate":
            ...
            print(f"media-guard-bank-attribution: WROTE {args.output.relative_to(ROOT)}")
        else:
            ...
            print(
                "media-guard-bank-attribution: PASS "
                "guard=380 vm_callprim-credit=217 alignment=9 bank-debit=172"
            )
    except (AttributionError, OSError, KeyError, ValueError, tarfile.TarError) as exc:
        print(f"media-guard-bank-attribution: FAIL: {exc}", file=sys.stderr)
        return 1
    return 0
