#!/usr/bin/env python3
"""Qualify v1.6 Item 1 in two varied fresh public-source clones."""
...
BUILD_COMMAND = ("make", "--no-print-directory", "workbench-product-v160")
AXES = (
    {
        "id": "v160-public-clean-a",
        "PYTHONHASHSEED": "16023",
        "SOURCE_DATE_EPOCH": "1041379200",
        "TZ": "Pacific/Pago_Pago",
        "UMASK": "027",
    },
    {
        "id": "v160-public-clean-b",
        "PYTHONHASHSEED": "160987654",
...
def authority(root: Path = ROOT) -> dict[str, Any]:
    value = load(root / AUTHORITY.relative_to(ROOT), "v1.6 public authority")
    ...
    require(
        ...
        "v1.6 public authority envelope drift")
...
def check_root(root: Path) -> dict[str, Any]:
    ...
    manifest = load(manifest_path, "v1.6 selected manifest")
    ...
    require(
        ...
        "v1.6 aggregate differs from sealed artifact set")
    source = load(root / PUBLIC_MANIFEST, "public source manifest")
    require(
    ...
    return {
        ...
        "public_source_manifest_sha256": sha(root / PUBLIC_MANIFEST),
...
def resolve_commit(repository: Path, revision: str) -> str:
    value = run(
        ...
        label="resolve v1.6 public source commit").strip()
    require(
        len(value) == 40
        and all(character in "0123456789abcdef" for character in value),
        "v1.6 public source commit is not a full commit identity")
    return value
...
def build_clone(parent: Path, repository: Path, commit: str,
    ...
    command = (
        "umask", axis["UMASK"], "&&", *BUILD_COMMAND,
    )
    output = run(
        ["sh", "-c", " ".join(command)], cwd=checkout, env=environment,
        label=f"{axis['id']} v1.6 public build")
    require(
        output.count("v1.6 public product: LINK PASS WPLTO=1 evidence=0") == 1,
        f"{axis['id']} does not prove exactly one WPLTO/link")
    require(
        output.count("v1.6 public product: COMPLETION PASS rebuilds=0") == 1,
        f"{axis['id']} Completion rebuilt product inputs")
    require(
        output.count("v1.6 public product: FULL PASS roles=22 evidence=0") == 1,
...
def validate_receipt(value: dict[str, Any]) -> None:
    ...
    require(set(value) == required, "v1.6 clean-build receipt schema drift")
    require(
        value["format"] == "lisp65-c2-v160-public-clean-build-receipt-v1"
        and value["version"] == 1 and value["status"] == "passed"
        and value["source_repository_role"]
            == "curated-public-source-candidate"
        and value["entry_point"] == "make workbench-product-v160"
        and value["private_evidence_inputs"] == 0
        and value["selected_variant"] == "v1.6-item-1-only"
        and value["artifact_count"] == 22
        and value["result"]
            == "two-varied-fresh-public-clones-reproduce-v1.6-item1-role-set",
        "v1.6 clean-build receipt envelope drift")
    require(
        isinstance(value["builds"], list) and len(value["builds"]) == 2
        and value["builds"][0]["environment"]
            != value["builds"][1]["environment"],
        "v1.6 receipt lacks two varied fresh builds")
    require(
        isinstance(value["artifacts"], list)
        and len(value["artifacts"]) == 22,
        "v1.6 receipt artifact inventory drift")
    for build in value["builds"]:
        require(
            ...
            "v1.6 receipt build identity or lifecycle drift")
    require(
        value["mutations"] == MUTATION_LEDGER,
        "v1.6 clean-build mutation ledger drift")
...
def mutation_proof(value: dict[str, Any]) -> dict[str, str]:
    ...
    for label, mutate in mutations:
        ...
        try:
            ...
            continue
        raise CleanBuildError(f"v1.6 clean-build mutation survived: {label}")
    return result
...
def qualify(repository: Path, revision: str, output: Path,
    ...
    require(
        (repository / ".git").exists() or (repository / ".git").is_file(),
        "v1.6 public source repository is not a Git checkout")
    require(
    ...
    with tempfile.TemporaryDirectory(
            prefix="lisp65-v160-public-clean-build-") as raw:
        builds = [
    ...
    for field in (
        ...
        require(
            ...
            f"varied v1.6 public builds diverged: {field}")
    value = {
        "format": "lisp65-c2-v160-public-clean-build-receipt-v1",
        "version": 1,
        "status": "passed",
        ...
        "public_source_manifest_sha256":
            first["public_source_manifest_sha256"],
        ...
        "claim_limit": (
            ...
            "Halt-A-selected v1.6 Item-1 artifact set. It does not publish, "
            "tag, release, or extend hardware acceptance claims."),
    }
    require(
        mutation_proof(value) == MUTATION_LEDGER,
        "v1.6 clean-build mutation proof drift")
...
def selftest() -> None:
    ...
    build = {
        ...
        "command": "make --no-print-directory workbench-product-v160",
        "environment": {
    ...
    value = {
        "format": "lisp65-c2-v160-public-clean-build-receipt-v1",
        "version": 1, "status": "passed", "measured_on": "2026-08-25",
        "source_commit": "e" * 40,
        "source_repository_role": "curated-public-source-candidate",
        "entry_point": "make workbench-product-v160",
        "private_evidence_inputs": 0,
        ...
        "selected_variant": "v1.6-item-1-only",
        "artifact_count": 22, "artifact_set_sha256": "a" * 64,
        "product_build_id": "12345678", "profile_build_id": "90abcdef",
    ...
    require(
        mutation_proof(value) == MUTATION_LEDGER,
        "v1.6 clean-build selftest mutation proof drift")
    validate_receipt(value)
    print("c2-v160-public-clean-build: SELFTEST PASS mutations=6 roles=22")
...
def main() -> int:
    ...
    try:
        if args.action == "selftest":
            ...
            print(
                "c2-v160-public-clean-build: LOCAL PASS "
            ...
            value = qualify(
                ...
                args.llvm_mos_root)
            print(
                "c2-v160-public-clean-build: QUALIFIED "
            ...
            value = load(receipt, "v1.6 public clean-build receipt")
            validate_receipt(value)
            print(
                "c2-v160-public-clean-build: RECEIPT PASS "
                f"builds=2 roles={value['artifact_count']} "
                f"set={value['artifact_set_sha256']}")
        return 0
    except (
        ...
        print(
            "c2-v160-public-clean-build: FIRST RED: " + str(error),
            file=sys.stderr)
        return 2
