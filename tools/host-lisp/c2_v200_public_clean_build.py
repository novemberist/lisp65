#!/usr/bin/env python3
"""Qualify v2.0.0 in two varied fresh public-source clones."""
...
AUTHORITY = Path("config/c2-v200-public-build-authority.json")
BUILD_COMMAND = ("make", "--no-print-directory", "workbench-product-v200")
AXES = (
    {
        "id": "v200-public-clean-a",
...
def authority(root: Path) -> dict[str, Any]:
    value = load(root / AUTHORITY, "v2.0 public authority")
    roles = value.get("sealed_roles")
    require(
        value.get("format") == "lisp65-c2-public-build-authority-v8"
        and value.get("release") == "v2.0.0"
        and value.get("selected_variant") == "v2.0-tier1-lossless-native-editor-stripped"
...
def artifact_projection(root: Path, manifest: dict[str, Any],
    ...
    require(
        ...
        and manifest.get("selector") == "v2.0-tier1-lossless-native-editor-stripped"
        and manifest.get("artifact_count") == len(expected)
        and isinstance(rows, list) and len(rows) == len(expected),
        "v2.0 selected manifest envelope drift")
    result: list[dict[str, Any]] = []
    for row in rows:
        require(
            isinstance(row, dict)
            and set(row) == {"role", "name", "path", "bytes", "sha256"},
            "v2.0 selected artifact schema drift")
        role = str(row["role"])
        ...
        require(
            ...
            f"v2.0 selected artifact binding drift: {role}")
        require(
            row["bytes"] == expected[role]["bytes"]
            and row["sha256"] == expected[role]["sha256"],
            f"v2.0 clean build differs from sealed role: {role}")
        result.append({key: row[key]
                       for key in ("role", "name", "bytes", "sha256")})
    require({row["role"] for row in result} == set(expected),
            "v2.0 clean-build role inventory drift")
    return sorted(result, key=lambda row: (row["role"], row["name"]))


def public_source_projection(root: Path) -> dict[str, Any]:
    """Bind the complete tracked source commit before archive packaging."""
    tracked = [line for line in run(
        ["git", "ls-files"], cwd=root, label="enumerate public source tree"
    ).splitlines() if line]
    require(tracked == sorted(set(tracked)) and tracked,
            "public tracked-source population drift")
    dirty = run(["git", "status", "--short", "--untracked-files=no"],
                cwd=root, label="check public source tree").strip()
    require(not dirty, "public clean build modified tracked source files")
    rows = []
    for name in tracked:
        path = root / name
        require(path.is_file() and not path.is_symlink(),
                f"public tracked source is not a regular file: {name}")
        rows.append({"path": name, "bytes": path.stat().st_size,
                     "sha256": sha(path)})
    tree = hashlib.sha256(canonical(rows)).hexdigest()
    manifest = {"format": "lisp65-public-source-projection-v1",
        "source_tree_clean": True, "file_count": len(rows),
        "tree_sha256": tree, "files": rows}
    return {"manifest_sha256": hashlib.sha256(canonical(manifest)).hexdigest(),
            "tree_sha256": tree, "file_count": len(rows)}


def check_root(root: Path) -> dict[str, Any]:
    auth = authority(root)
    manifest_path = root / str(auth["candidate_manifest_path"])
    manifest = load(manifest_path, "v2.0 selected manifest")
    ...
    require(
        ...
        "v2.0 aggregate differs from sealed artifact set")
    source = public_source_projection(root)
    return {
        "artifact_set_sha256": manifest["artifact_set_sha256"],
        "product_build_id": manifest["product_build_id"],
        "profile_build_id": manifest["profile_build_id"],
        "manifest_sha256": sha(manifest_path),
        "public_source_manifest_sha256": source["manifest_sha256"],
...
def resolve_commit(repository: Path, revision: str) -> str:
    value = run(
        ...
        label="resolve v2.0 public source commit").strip()
    require(len(value) == 40
            and all(character in "0123456789abcdef" for character in value),
            "v2.0 public source commit is not a full commit identity")
    return value
...
def build_clone(parent: Path, repository: Path, commit: str,
    ...
    output = run(
        ...
        label=f"{axis['id']} v2.0 public build")
    require(output.count(
        "v2.0 public product: LINK PASS WPLTO=1 evidence=0") == 1,
        f"{axis['id']} does not prove exactly one WPLTO/link")
    require(output.count(
        "v2.0 public product: MEDIA PASS roles=19") == 1,
        f"{axis['id']} Completion rebuilt product inputs")
    require(output.count(
        "v2.0 public product: FULL PASS roles=19 evidence=0") == 1,
...
def validate_receipt(value: dict[str, Any]) -> None:
    ...
    require(set(value) == required, "v2.0 clean-build receipt schema drift")
    require(
        value["format"] == "lisp65-c2-v200-public-clean-build-receipt-v1"
        ...
        and value["selected_variant"] == "v2.0-tier1-lossless-native-editor-stripped"
        and value["artifact_count"] == 19
        and value["result"]
            == "two-varied-fresh-public-clones-reproduce-v2.0-role-set",
        "v2.0 clean-build receipt envelope drift")
    require(isinstance(value["builds"], list) and len(value["builds"]) == 2
            and value["builds"][0]["environment"]
                != value["builds"][1]["environment"],
            "v2.0 receipt lacks two varied fresh builds")
    require(isinstance(value["artifacts"], list)
            and len(value["artifacts"]) == 19,
            "v2.0 receipt artifact inventory drift")
    for build in value["builds"]:
        require(
            ...
            "v2.0 receipt build identity or lifecycle drift")
    require(value["mutations"] == MUTATION_LEDGER,
            "v2.0 clean-build mutation ledger drift")
...
def mutation_proof(value: dict[str, Any]) -> dict[str, str]:
    ...
    for label, mutate in mutations:
        ...
        try:
            ...
            continue
        raise CleanBuildError(f"v2.0 clean-build mutation survived: {label}")
    return result
...
def qualify(repository: Path, revision: str, output: Path,
    ...
    require((repository / ".git").exists(),
            "v2.0 public source repository is not a Git checkout")
    ...
    with tempfile.TemporaryDirectory(
            prefix="lisp65-v200-public-clean-build-") as raw:
    ...
    for field in ("artifact_set_sha256", "product_build_id",
        ...
        require(first[field] == second[field],
                f"varied v2.0 public builds diverged: {field}")
    value = {
        "format": "lisp65-c2-v200-public-clean-build-receipt-v1",
        ...
        "result": "two-varied-fresh-public-clones-reproduce-v2.0-role-set",
        "claim_limit": (
            "This receipt qualifies public source reproduction of the "
            "Halt-A-selected v2.0 product. It does not publish, tag, release, "
            "or extend hardware acceptance claims."),
    }
    require(mutation_proof(value) == MUTATION_LEDGER,
            "v2.0 clean-build mutation proof drift")
...
def selftest() -> None:
    ...
    build = {
        ...
        "command": "make --no-print-directory workbench-product-v200",
    ...
    value["builds"][1]["environment"] = {
        ...
        "TZ": "B", "UMASK": "077"}
    require(mutation_proof(value) == MUTATION_LEDGER,
            "v2.0 clean-build selftest mutation proof drift")
    validate_receipt(value)
    print("c2-v200-public-clean-build: SELFTEST PASS mutations=6 roles=19")
...
def main() -> int:
    ...
    try:
        if args.action == "selftest":
            ...
            print("c2-v200-public-clean-build: LOCAL PASS "
            ...
            print("c2-v200-public-clean-build: QUALIFIED "
            ...
            value = load(receipt, "v2.0 public clean-build receipt")
            validate_receipt(value)
            print("c2-v200-public-clean-build: RECEIPT PASS "
                  f"builds=2 roles={value['artifact_count']} "
                  f"set={value['artifact_set_sha256']}")
        return 0
    except (CleanBuildError, OSError, ValueError, KeyError,
            subprocess.SubprocessError, json.JSONDecodeError) as error:
        print("c2-v200-public-clean-build: FIRST RED: " + str(error),
              file=sys.stderr)
        return 2
