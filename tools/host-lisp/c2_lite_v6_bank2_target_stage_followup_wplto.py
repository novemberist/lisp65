#!/usr/bin/env python3
"""Qualify the approved Bank-2 stage placement correction with one WPLTO."""
...
import c2_lite_v6_bank2_target_stage_wplto as B


ROOT = B.ROOT
OUT = ROOT / "build/c2-lite/v6-bank2-target-stage-followup-wplto"
RECEIPT = B.EVIDENCE / (
    "c2.2-c2-lite-v6-bank2-target-stage-followup-wplto-receipt.json")
DIAGNOSIS = B.EVIDENCE / (
    "c2.2-c2-lite-v6-bank2-target-stage-wplto-first-red-diagnosis.json")

B.OUT = OUT
B.RECEIPT = RECEIPT
_base_authority = B.authority
...
def authority() -> dict[str, Any]:
    ...
    approved = contract["bank2_target_stage_successor_authorization"][
        "approved_wplto_first_red"]
    B.require(approved["phase03b_over_cap_bytes"] == 55
              and approved["resident_rodata_delta_bytes"] == 213
              and "one follow-up WPLTO" in approved["disposition"],
              "Bank-2 follow-up WPLTO authority absent")
    ...
    B.require(diagnosis["status"].startswith("FIRST RED:")
              ...
              and diagnosis["scope"]["hardware_runs"] == 0,
              "Bank-2 target-stage First Red is not authoritative")
    value["followup_driver"] = B.bind(Path(__file__))
    value["approved_first_red_diagnosis"] = B.bind(DIAGNOSIS)
    return value
...
def source_gate(decoder_path: Path = B.DECODER,
    ...
    checks = {
        "record_phase_owns_target_cross_binding":
            "code_target = 0u" in phase02a
            and "i * 32u + 18u" in phase02a
            and "r24(target) != code_target" in phase02a
            and "code_target += r16(s + 11)" in phase02a
            and "code_target != 34403UL" in phase02a,
        ...
        "exact_static_plane_rechecked": "next != 34403UL" in phase03b,
    ...
    B.require(all(checks.values()), "Bank-2 follow-up source contract red: "
              + str([name for name, ok in checks.items() if not ok]))

    if not test_mutations:
        return {"status": "passed-semantic-placement-source-contract",
    ...
    mutations = {
        ...
        "cutpoint-replayed": ("decoder", decoder.replace(
        ...
        "static-plane-close-bypassed": ("decoder", decoder.replace(
            "if (code_target != 34403UL)", "if (0u)", 1)),
    ...
    B.require(len(rejected) == len(mutations),
              "Bank-2 follow-up mutation matrix incomplete")
    return {"status": "passed-semantic-placement-source-contract",
