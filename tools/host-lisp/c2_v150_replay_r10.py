#!/usr/bin/env python3
"""Run the authorized Link-97 r10 replay on the current-schema closure."""
...
def configure_replay() -> None:
    CARD.REPLAY_PREVIOUS_RED = R9
    CARD.REPLAY = R10
    CARD.REPLAY_PROFILE = R10 / "candidate-profile.json"
    CARD.REPLAY_INTERNAL = R10 / "wplto-internal.json"
    CARD.REPLAY_LINKED_GATE = R10 / "single-submit-linked-gates.json"
    CARD.REPLAY_FIRST_RED_RECEIPT = R9_FIRST_RED
...
def complete_in_fresh_process() -> None:
    ...
    CARD.require(result.returncode == 0,
                 "v1.5 r10 fresh-process completion red:\n" + result.stdout)
...
def selftest() -> int:
    configure_replay()
    CARD.require(
        R9.is_dir() and (R9 / "candidate-profile.json").is_file()
        and R9_FIRST_RED.is_file()
        and not R10.exists()
        and CARD.AMBIENT.RECEIPT == CARD.AMBIENT.SCHEMA_RECEIPT
        and CARD.AMBIENT.SCHEMA_RECEIPT.is_file()
        and THREE_FIELD_MAP.is_file()
        and CARD.REPLAY_AUTHORIZATIONS[-1] == AUTHORIZATION,
        "v1.5 r10 replay adapter boundary red")
    CARD.selftest()
    print("v1.5 Link-97 r10 replay adapter selftest: PASS")
    return 0
...
def complete() -> int:
    ...
    # Bind candidate identity only.  The inherited closer owns the sole
    # configure_card()/one-shot profile selection in this fresh process.
...
if __name__ == "__main__":
    try:
        ...
        print(f"v1.5 Link-97 r10 replay: FIRST RED: {error}")
        raise SystemExit(2)
