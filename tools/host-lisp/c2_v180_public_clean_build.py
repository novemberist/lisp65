#!/usr/bin/env python3
"""Qualify v1.8.0 in two varied fresh public-source clones."""
...
AUTHORITY = Path("config/c2-v180-public-build-authority.json")
PUBLIC_MANIFEST = Path("PUBLIC-SOURCE-MANIFEST.json")
BUILD_COMMAND = ("make", "--no-print-directory", "workbench-product-v180")
AXES = (
    {
        "id": "v180-public-clean-a",
...
def authority(root: Path) -> dict[str, Any]:
    value = load(root / AUTHORITY, "v1.8 public authority")
    roles = value.get("sealed_roles")
    require(
        value.get("format") == "lisp65-c2-public-build-authority-v6"
        ...
        "v1.8 public authority envelope drift")
...
def check_root(root: Path) -> dict[str, Any]:
    ...
    manifest = load(manifest_path, "v1.8 selected manifest")
    ...
    require(
        ...
        "v1.8 aggregate differs from sealed artifact set")
...
def resolve_commit(repository: Path, revision: str) -> str:
    value = run(
        ...
        label="resolve v1.8 public source commit").strip()
    require(len(value) == 40
            and all(character in "0123456789abcdef" for character in value),
            "v1.8 public source commit is not a full commit identity")
    return value
...
def build_clone(parent: Path, repository: Path, commit: str,
    ...
    output = run(
        ...
        label=f"{axis['id']} v1.8 public build")
    require(output.count(
        "v1.8 public product: LINK PASS WPLTO=1 evidence=0") == 1,
        f"{axis['id']} does not prove exactly one WPLTO/link")
    require(output.count(
        "v1.8 public product: COMPLETION PASS rebuilds=0") == 1,
        f"{axis['id']} Completion rebuilt product inputs")
    require(output.count(
        "v1.8 public product: FULL PASS roles=22 evidence=0") == 1,
...
def validate_receipt(value: dict[str, Any]) -> None:
    ...
    require(set(value) == required, "v1.8 clean-build receipt schema drift")
    require(
        value["format"] == "lisp65-c2-v180-public-clean-build-receipt-v1"
        and value["version"] == 1 and value["status"] == "passed"
        and value["source_repository_role"]
            == "curated-public-source-candidate"
        and value["entry_point"] == "make workbench-product-v180"
        and value["private_evidence_inputs"] == 0
        and value["selected_variant"] == "v1.8-closed-capture-substrate"
        and value["artifact_count"] == 22
        and value["result"]
            == "two-varied-fresh-public-clones-reproduce-v1.8-role-set",
        "v1.8 clean-build receipt envelope drift")
    require(isinstance(value["builds"], list) and len(value["builds"]) == 2
            and value["builds"][0]["environment"]
                != value["builds"][1]["environment"],
            "v1.8 receipt lacks two varied fresh builds")
    require(isinstance(value["artifacts"], list)
            and len(value["artifacts"]) == 22,
            "v1.8 receipt artifact inventory drift")
    for build in value["builds"]:
        require(
            ...
            "v1.8 receipt build identity or lifecycle drift")
    require(value["mutations"] == MUTATION_LEDGER,
            "v1.8 clean-build mutation ledger drift")
...
def mutation_proof(value: dict[str, Any]) -> dict[str, str]:
    ...
    for label, mutate in mutations:
        ...
        try:
            ...
            continue
        raise CleanBuildError(f"v1.8 clean-build mutation survived: {label}")
    return result
...
def qualify(repository: Path, revision: str, output: Path,
    ...
    require((repository / ".git").exists(),
            "v1.8 public source repository is not a Git checkout")
    ...
    with tempfile.TemporaryDirectory(
            prefix="lisp65-v180-public-clean-build-") as raw:
    ...
    for field in ("artifact_set_sha256", "product_build_id",
        ...
        require(first[field] == second[field],
                f"varied v1.8 public builds diverged: {field}")
    value = {
        "format": "lisp65-c2-v180-public-clean-build-receipt-v1",
        ...
        "result": "two-varied-fresh-public-clones-reproduce-v1.8-role-set",
        "claim_limit": (
            "This receipt qualifies public source reproduction of the "
            "Halt-A-selected v1.8 product. It does not publish, tag, release, "
            "or extend hardware acceptance claims."),
    }
    require(mutation_proof(value) == MUTATION_LEDGER,
            "v1.8 clean-build mutation proof drift")
...
def selftest() -> None:
    ...
    build = {
        ...
        "command": "make --no-print-directory workbench-product-v180",
    ...
    value["builds"][1]["environment"] = {
        ...
        "TZ": "B", "UMASK": "077"}
    require(mutation_proof(value) == MUTATION_LEDGER,
            "v1.8 clean-build selftest mutation proof drift")
    validate_receipt(value)
    print("c2-v180-public-clean-build: SELFTEST PASS mutations=6 roles=22")
...
def main() -> int:
    ...
    try:
        if args.action == "selftest":
            ...
            print("c2-v180-public-clean-build: LOCAL PASS "
            ...
            print("c2-v180-public-clean-build: QUALIFIED "
            ...
            value = load(receipt, "v1.8 public clean-build receipt")
            validate_receipt(value)
            print("c2-v180-public-clean-build: RECEIPT PASS "
                  f"builds=2 roles={value['artifact_count']} "
                  f"set={value['artifact_set_sha256']}")
        return 0
    except (CleanBuildError, OSError, ValueError, KeyError,
            subprocess.SubprocessError, json.JSONDecodeError) as error:
        print("c2-v180-public-clean-build: FIRST RED: " + str(error),
              file=sys.stderr)
        return 2
