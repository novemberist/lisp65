#!/usr/bin/env python3
"""Prepare, verify, and record the sole Link-107 D1 boot contact."""
...
CONFIG = ROOT / "config/c2-v150-v21-dependent-vma-far-device-session.json"
RUNNER = ROOT / "scripts/c2-v21-dependent-vma-d1-hw.sh"
MEDIA = ROOT / (
    "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
    "c2.3-v2.1-dependent-vma-media-receipt.json")
SUMMARY = ROOT / (
    "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
    "c2.3-v2.1-dependent-vma-completion-media-receipt.json")
PREP = ROOT / (
    "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
    "c2.3-v2.1-dependent-vma-d1-preparation-receipt.json")
RESULT = ROOT / (
    "tests/bytecode/dialect-v2/evidence/architecture-blocks/"
    "c2.3-v2.1-dependent-vma-d1-receipt.json")
OUT = ROOT / "build/c2.3/v2.1-dependent-vma-d1"
...
def contract(value: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    require(
        ...
        and value.get("authority", {}).get("media_closure")
        ...
        "Link 107 D1 contract/media authority drift")
    return value
...
def runner_gate(source_override: str | None = None) -> dict[str, Any]:
    ...
    require(
        ...
        "Link 107 D1 runner lifecycle token absent")
    ...
    require(
        ...
        "Link 107 D1 liveness window admits device access")
    require(all(f"grep -Fqx '{sign}'" in source for sign in SIGNS),
            "Link 107 D1 can omit owner liveness confirmation")
...
def mutations(config: dict[str, Any], source: str) -> list[str]:
    ...
    require(rejected == list(cases), "Link 107 D1 mutation survived")
    return rejected
...
def derive_preparation() -> dict[str, Any]:
    ...
    return {
        "format": "lisp65-c2.3-v2.1-dependent-vma-d1-preparation-v1",
        "recorded_on": "2026-08-15",
        "status": "V21-LINK107-D1-PREPARED-NOT-RUN",
        ...
        "claim_limit": (
            "Fresh Link 107 D1 only. D2-D5 remain closed until the owner "
...
def validate_preparation(value: dict[str, Any], *, verify: bool) -> None:
    require(
        value.get("status") == "V21-LINK107-D1-PREPARED-NOT-RUN"
        ...
        "Link 107 D1 preparation claim drift")
    if verify:
        require(value == derive_preparation(), "Link 107 D1 preparation stale")
...
def verify_terminal(text: Path, image: Path) -> dict[str, Any]:
    ...
    require("WORKBENCH 1.5.0" in raw and "lisp65>" in raw,
            "Link 107 D1 banner/prompt absent")
...
def derive_result() -> dict[str, Any]:
    ...
    require(signs == SIGNS and (OUT / "owner-visible-signs-confirmed").is_file(),
            "Link 107 D1 owner liveness confirmation absent")
    return {
        "format": "lisp65-c2.3-v2.1-dependent-vma-d1-v1",
        "recorded_on": "2026-08-15",
        "status": "V21-LINK107-D1-GREEN",
        ...
        "execution_accounting": {"hardware_contacts": 1, "forms": 0,
            "product_links": 0, "WPLTO_runs": 0},
        "unlock": {"D2_D5": True},
        "claim_limit": "Fresh Link 107 D1 only; D2-D5 have not run.",
    }
...
def main() -> int:
    ...
    if args.action == "prepare":
        require(not PREP.exists(), "Link 107 D1 preparation exists")
        PREP.write_bytes(canonical(derive_preparation()))
        print("Link 107 D1 preparation: PASS mutations=7")
    elif args.action == "check":
        validate_preparation(load(PREP), verify=True)
        print("Link 107 D1 check: PASS mutations=7")
    elif args.action == "selftest":
        validate_preparation(derive_preparation(), verify=False)
        print("Link 107 D1 selftest: PASS mutations=7")
        ...
        print("Link 107 D1 terminal: PASS")
    else:
        require(not RESULT.exists(), "Link 107 D1 result exists")
        RESULT.write_bytes(canonical(derive_result()))
        print("Link 107 D1: PASS D2-D5 now permitted")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"LINK 107 D1: {message}", file=sys.stderr)
        raise SystemExit(1)
