#!/usr/bin/env python3
"""Rebind the nonpromotable C1 carrier to immutable Link 62."""
...
LINK_RECEIPT_STATUS = "passed-link62-pure-full-replay-all-gates-green"
...
def main() -> int:
    ...
    session = json.loads(
        (LINK / "runtime-overlays-session-final.json").read_text(
            encoding="utf-8"))["storage"]
    value["format"] = (
        "lisp65-c2.2-Link62-C1-Freezer-v4-carrier-receipt-v1")
    value["status"] = (
        "passed-Link62-capacity-and-gates-awaiting-hardware")
    value["authority"]["link62_rebind_driver"] = BASE.bind(Path(__file__))
    value["proof"]["boot_inventory"] = {
        ...
        "source": "Link62 pure full replay",
    }
    ...
    value["next_gate"] = (
        "one Link62 hardware appointment: Cutpoint 3 with episode latch, "
    ...
    print(
        "c2-c1-freezer-carrier-link62: PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-c1-freezer-carrier-link62: FIRST RED: " + str(error),
            file=sys.stderr,
        )
        raise SystemExit(2)
