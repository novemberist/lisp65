#!/usr/bin/env python3
"""Give the verified Link-66 C1 carrier its final artifact identity."""
...
import c2_c1_freezer_carrier_link66 as LINK66  # noqa: E402
...
MAIN = OUT / "runtime-overlays-session-c1-freezer-link66-stage-bound.bin"
REGION1 = OUT / "runtime-overlays-session-c1-freezer-link66-region1.bin"
MANIFEST = OUT / (
    "runtime-overlays-session-c1-freezer-link66-stage-bound.json")
...
def main() -> int:
    ...
    require(
        ...
        "unfinished Link-66 carrier authority drift")
    ...
    manifest["format"] = (
        "lisp65-C1-Freezer-Link66-v4-rebound-stage-bound-family-v1")
    manifest["status"] = (
        "passed-nonpromotable-Link66-carrier-awaiting-hardware")
    ...
    os.chmod(OUT, 0o555)
    print(
        "c2-c1-freezer-carrier-link66-artifact-completion: PASS "
...
if __name__ == "__main__":
    try:
        ...
        print(
            "c2-c1-freezer-carrier-link66-artifact-completion: FIRST RED: "
            + str(error),
            file=sys.stderr)
        raise SystemExit(2)
