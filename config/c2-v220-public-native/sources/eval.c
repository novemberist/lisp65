...
static obj lcc_install_obj(obj fnlist, obj defname) {
    gc_rootsp_t root_base = gc_rootsp;
    obj result;
    if (!GC_CAN_RESERVE(2)) {
...
static obj append2(obj a, obj b) {
    ...
    /* Recurses once per element of `a` and holds 3 root slots across the
     * recursion, so a long enough list walks off the root stack.  Same resource
     * and same refusal as eval_env's depth guard -- no new error code. */
    if (!GC_CAN_RESERVE(3)) { lisp_abort_static(LISP65_ERR_RECURSION_TOO_DEEP, "recursion too deep"); return NIL; }
    base = gc_rootsp;
    GC_PUSH(a); GC_PUSH(b);
    r = append2(cdr(a), b);
    GC_PUSH(r);
...
static obj qq_list(obj x, obj env, uint8_t d) {
    ...
    /* qq_list recurses once per list ELEMENT (not per nesting level) and keeps
     * 3 root slots live across that recursion.  For a list of atoms it never
     * calls eval_env, so eval_env's depth guard never sees it: without this
     * check `(1 1 1 ...) walked past gc_rootstack at 42 elements with
     * GC_ROOTS=128 (UBSan: index 128 out of bounds).  Worst case per level is 4
     * slots (x, env, r1, r2). */
    if (!GC_CAN_RESERVE(4)) { lisp_abort_static(LISP65_ERR_RECURSION_TOO_DEEP, "recursion too deep"); return NIL; }
    base = gc_rootsp;
...
static obj eval_vm_bridge(obj sym, const obj *args, uint8_t n) {
    ...
    HB(6); LA(10);  /* J: bridge */
    gc_rootsp_t abase = gc_rootsp;
    uint8_t i;
    if (fn == NIL) {
        lisp_abort_static_symbol(LISP65_ERR_UNDEFINED_FUNCTION, sym, undef_msg(sym)); return NIL;
    }
    /* Same accounting as the VM's apply/funcall arm: n args plus the list slot.
     * The bridge is entered from CALLPRIM, which may leave the root stack full. */
