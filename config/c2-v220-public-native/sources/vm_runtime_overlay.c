...
/* Retention is valid only while loaded_len remains nonzero. Every existing
 * wipe retires this state. Only the designated session member may publish
 * it; other owners still leave an empty window. */
#ifndef CARD2B_RETAIN_SLOT
#define CARD2B_RETAIN_SLOT 52u
#endif
static uint16_t card2b_retained_entry, card2b_retained_crc;
...
#if defined(LISP65_RTOV_CRC_CONVERGENCE) || defined(LISP65_C2_LITE_CHIP_RAM)
...
vm_runtime_overlay_status vm_runtime_overlay_exec_family(
    ...
    /* Do not bypass family/generation, busy, fault, argument or stack checks.
     * A raw poke into an idle retained window is caught by this CRC before
     * entry. Chip-RAM retains the existing single-check/fail-closed policy. */
    if (slot == CARD2B_RETAIN_SLOT &&
        rtov_family == LISP65_RUNTIME_OVERLAY_FAMILY_SESSION &&
        !RTOV_TRANSACTION_ACTIVE() && rtov_loaded_len) {
        if (rtov_crc_mem((const uint8_t *)RTOV_TARGET, rtov_loaded_len) !=
            card2b_retained_crc)
            return rtov_fail(VM_RUNTIME_OVERLAY_ERR_CRC);
        entry = card2b_retained_entry;
        goto card2b_invoke;
    }
    ...
    if (rtov_crc_mem((const uint8_t *)RTOV_TARGET, rtov_loaded_len) !=
        ...
        return rtov_fail(VM_RUNTIME_OVERLAY_ERR_CRC);
#endif
    card2b_retained_entry = entry;
    card2b_retained_crc = verify.payload_crc;
card2b_invoke:
