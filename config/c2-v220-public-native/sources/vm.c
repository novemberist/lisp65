...
/* Proposed private fixed context, not a new primitive ID or public name. */
typedef struct { obj *args; uint8_t nargs; obj result; } card2b_disk_context;
static card2b_disk_context card2b_disk_frame;
static __attribute__((always_inline)) inline obj card2b_disk_body(obj *a, uint8_t n) {
        if (n == 0) {
...
__attribute__((noinline,used,section(".lisp65_rt_card2b_disk")))
uint8_t card2b_disk_entry(void *opaque) {
    card2b_disk_context *c=opaque;
    vm_status=VM_OK;
    c->result=card2b_disk_body(c->args,c->nargs);
    return vm_status;
}
static __attribute__((noinline)) obj card2b_disk_call(obj *a,uint8_t n) {
    card2b_disk_frame.args=a;card2b_disk_frame.nargs=n;
    (void)vm_runtime_overlay_exec(52,&card2b_disk_frame,&vm_status);
    return card2b_disk_frame.result;
}
...
static __attribute__((noinline)) obj vm_callprim(uint8_t pid, obj *a, uint8_t n) {
    ...
    case 21:  /* %disk-poke */
        ...
        return a[1];
    case 22: return card2b_disk_call(a,n);
#endif
#endif
