...
static obj read_expr_1(void) {
     ...
     * which is ASCII backslash.  Outside a string that code is quasiquote
     * sugar — the typeable spelling of '`' on a MEGA65 keyboard.  Inside a
     * string literal backslash keeps its escape meaning; read_string() runs
     * its own scan from the opening '"' and never consults this branch. */
    if (c == '\\') {
        rd_next();
        r = sugar("quasiquote");
        goto done;
